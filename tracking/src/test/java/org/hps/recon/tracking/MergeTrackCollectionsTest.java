...
public class MergeTrackCollectionsTest extends TestCase {
    ...
    public void testMerging() throws Exception {
        ...
        File lcioInputFile;
        File trackFile;
        File outputFile = new TestOutputFile(testFileName.replaceAll(".slcio", "") + "_MergeTest2.slcio");
        outputFile.getParentFile().mkdirs(); // make sure the parent directory
                                             // exists

        // Create tracks
        if (createFirstTracks) {
            trackFile = new TestOutputFile(testFileName.replaceAll(".slcio", "") + "_MergeTest1.slcio");
    ...
    static class MergePlotsDriver extends Driver {
        ...
        private final IHistogram1D numTracksNewGood = aida.histogram1D("numTracksNewGood", 10, 0, 10);
        private final IHistogram1D numHitsNewGood = aida.histogram1D("numHitsNewGood", 10, 0, 10);
        ...
        protected void endOfData() {
            ...
            File outputFile2 = new TestOutputFile(testFileName.replaceAll(".slcio", "") + "_MergeTest.aida");
            outputFile2.getParentFile().mkdirs();
            try {
                aida.saveAs(outputFile2);
            } catch (IOException ex) {
                Logger.getLogger(MergeTrackCollectionsTest.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
