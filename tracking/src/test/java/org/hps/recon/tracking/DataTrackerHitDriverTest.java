...
public class DataTrackerHitDriverTest extends TestCase {
    ...
    public static void testIt() throws Exception {
        ...
        RawTrackerHitSensorSetup sensorSetup = new RawTrackerHitSensorSetup();
        String[] readoutCollections = {"SVTRawTrackerHits"};
        sensorSetup.setReadoutCollections(readoutCollections);
        ...
        dthDriver.setSetupSensors(rerunFromFittedHits);
        dthDriver.setDebug(true);
        if (!rerunFromFittedHits) {
            loop.add(sensorSetup);
            loop.add(rthfDriver);
