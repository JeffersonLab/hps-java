    ...
    public void process(EventHeader event) {
        ...
        for (Track trk : tracklist) {
            ...
            if (1 == 1) {
                ...
                aida.histogram1D("Track sin(phi0)", 50, -0.5, 0.5).fill(
                        Math.sin(trk.getTrackStates().get(0).getParameter(1)));
                ...
                for (int i = 1; i < 11; i++) {
                    ...
                    if (i == 3 && Math.sin(trk.getTrackStates().get(0).getParameter(1)) > 0) {
                        aida.histogram1D("Positive phi0  deltaU -- Layer " + mylayer, 50, -duRange, duRange).fill(
                                res[0]);
                        aida.histogram1D("Positive phi0 deltaU Pull-- Layer " + mylayer, 50, -3, 3).fill(
                                res[0] / res[3]);
                    }
                    if (i == 3 && Math.sin(trk.getTrackStates().get(0).getParameter(1)) < 0) {
                        aida.histogram1D("Negative phi0  deltaU -- Layer " + mylayer, 50, -duRange, duRange).fill(
                                res[0]);
                        aida.histogram1D("Negative phi0 deltaU Pull-- Layer " + mylayer, 50, -3, 3).fill(
                                res[0] / res[3]);
                    }
