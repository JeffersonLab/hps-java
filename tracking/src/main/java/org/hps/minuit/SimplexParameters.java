...
class SimplexParameters {
    ...
    void update(double y, MnAlgebraicVector p) {
        ...
        if (y < theSimplexParameters.get(jl()).first) {
            theJLow = jh();
        }

        int jh = 0;
        for (int i = 1; i < theSimplexParameters.size(); i++) {
            if (theSimplexParameters.get(i).first > theSimplexParameters.get(jh).first) {
                jh = i;
            }
        }
        theJHigh = jh;
    ...
    MnAlgebraicVector dirin() {
        ...
        for (int i = 0; i < theSimplexParameters.size() - 1; i++) {
            ...
            for (int j = 0; j < theSimplexParameters.size(); j++) {
                if (theSimplexParameters.get(j).second.get(i) < plit) {
                    plit = theSimplexParameters.get(j).second.get(i);
                }
                if (theSimplexParameters.get(j).second.get(i) > pbig) {
                    pbig = theSimplexParameters.get(j).second.get(i);
                }
            }
