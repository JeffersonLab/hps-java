...
public class GblTrajectory {
     ...
     * scattering is added by offsets in a local system. Along the initial trajectory points are defined with can
     * describe a measurement or a (thin) scatterer or both. Measurements are arbitrary functions of the local track
     * parameters at a point (e.g. 2D: position, 4D: direction+position). The refit provides corrections to the local
     * track parameters (in the local system) and the corresponding covariance matrix at any of those points.
     * Non-diagonal covariance matrices will be diagonalized internally. Outliers can be down-weighted by use of
     * M-estimators. The broken lines trajectory is defined by (2D) offsets at the first and last point and all points
     * with a scatterer. The prediction for a measurement is obtained by interpolation of the enclosing offsets and for
     * triplets of adjacent offsets kink angles are determined. This requires for all points the jacobians for
     * propagation to the previous and next offset. These are calculated from the point-to-point jacobians along the
     * initial trajectory. The sequence of points has to be strictly monotonic in arc-length. Additional local or global
     * parameters can be added and the trajectories can be written to special binary files for calibration and alignment
     * with Millepede-II. (V. Blobel, NIM A, 566 (2006), pp. 5-13). Besides simple trajectories describing the path of a
     * single particle composed trajectories are supported. These are constructed from the trajectories of multiple
     * particles and some external parameters (like those describing a decay) and transformations at the first points
     * from the external to the local track parameters. The conventions for the coordinate systems follow: Derivation of
     * Jacobians for the propagation of covariance matrices of track parameters in homogeneous magnetic fields A.
     * Strandlie, W. Wittek, NIM A, 566 (2006) 687-698. \section call_sec Calling sequence -# Create list of points on
     * initial trajectory:\n <tt>List<GblPoint> list</tt> -# For all points on initial trajectory: - Create points and
     * add appropriate attributes:\n - <tt>point = gbl::GblPoint(..)</tt> - <tt>point.addMeasurement(..)</tt> - Add
     * additional local or global parameters to measurement:\n - <tt>point.addLocals(..)</tt> -
     * <tt>point.addGlobals(..)</tt> - <tt>point.addScatterer(..)</tt> - Add point (ordered by arc length) to list:\n
     * <tt>list.add(point)</tt> -# Create (simple) trajectory from list of points:\n
     ...
     * <tt>if (not traj.isValid()) .. //abort</tt> -# Fit trajectory, return error code, get Chi2, Ndf (and weight lost
     * by M-estimators):\n <tt>ierr = traj.fit(..)</tt> -# For any point on initial trajectory: - Get corrections and
     * covariance matrix for track parameters:\n <tt>[..] = traj.getResults(label)</tt> -# Optionally write trajectory
     * to MP binary file:\n <tt>traj.milleOut(..)</tt> \section loc_sec Local system and local parameters At each point
     * on the trajectory a local coordinate system with local track parameters has to be defined. The first of the five
     * parameters describes the bending, the next two the direction and the last two the position (offsets). The
     * curvilinear system (T,U,V) with parameters (q/p, lambda, phi, x_t, y_t) is well suited. \section impl_sec
     * Implementation Matrices are implemented with ROOT (root.cern.ch). User input or output is in the form of
     * TMatrices. Internally SMatrices are used for fixes sized and simple matrices based on List<> for variable sized
     * matrices. \section ref_sec References - V. Blobel, C. Kleinwort, F. Meier, Fast alignment of a complex tracking
     * detector using advanced track models, Computer Phys. Communications (2011), doi:10.1016/j.cpc.2011.03.017 - C.
     * Kleinwort, General Broken Lines as advanced track fitting method, NIM A, 673 (2012), 107-110,
