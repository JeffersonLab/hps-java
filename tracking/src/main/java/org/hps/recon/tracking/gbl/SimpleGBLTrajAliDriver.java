...
public class SimpleGBLTrajAliDriver extends Driver {
    ...
    protected void process(EventHeader event) {
        ...
        for (Track track : tracks) {
            ...
            if (useParticles) {
                ...
                aidaGBL.histogram3D(eopFolder + "EoP_vs_tanLambda_phi").fill(trk_prms[BaseTrack.TANLAMBDA], trk_prms[BaseTrack.PHI], momC / trackp);
                ...
                if (TriggerModule.inFiducialRegion(em_cluster)) {
                    ...
                    aidaGBL.histogram3D(eopFolder + "EoP_vs_tanLambda_phi_fid").fill(trk_prms[BaseTrack.TANLAMBDA], trk_prms[BaseTrack.PHI], momC / trackp);
            ...
            if (constrainedFit) {
                ...
                if (posEoP > 0 && (track.getCharge() < 0)) {
                    ...
                    double p = sqrt(track.getPX() * track.getPX() + track.getPY() * track.getPY() + track.getPZ() * track.getPZ());
            ...
            List<GBLStripClusterData> trackGblStripClusterData = computeGBLStripClusterData(track, TrackType, temp, gblStripClusterDataRelations, event);
    ...
    // Get the derivatives
    private void addGlobalDerivatives(List<Double> derBuff, List<Integer> labBuff, Hep3Matrix Rgtosc, Hep3Vector Tgtosc, Matrix g_ders,
    ...
    */
    List<GBLStripClusterData> computeGBLStripClusterData(Track track, int TrackType, List<TrackerHit> seedTrackerStripHits, String gblSCDR, EventHeader event) {
    ...
    private void ComputeStructureDerivatives(GblPointJna gblpoint) {
        ...
        if (debugAlignmentDs)
            System.out.printf("PF:: The sensor vol %s and mpid %s corresponds to %s is contained in the structure %s\n\n\n", volname, mpid, mysensor.getName(),
                    myade_Mod.getName());

        // Modules
        addGlobalDerivatives(derBuff, labBuff, Rgtos, Tgtos, g_ders, myade_Mod, labels);
        ...
        // UChannels
        addGlobalDerivatives(derBuff, labBuff, Rgtos, Tgtos, g_ders, myade_UC, labels);
