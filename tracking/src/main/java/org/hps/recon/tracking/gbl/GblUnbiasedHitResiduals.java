...
public class GblUnbiasedHitResiduals extends Driver {
    ...
    private boolean fullyUnbiased = true;

    //Output File name
    private String outputFileName = "Residuals.root";
    ...
    Map<String, IHistogram2D> residualYvsCluSize   = new HashMap<String,IHistogram2D>();
    Map<String, IHistogram2D> b_residualYvsCluSize = new HashMap<String,IHistogram2D>();
    ...
    double b_maxYRes = 0.5*maxYRes;
    double b_minYRes = -0.5*maxYRes;
    ...
    public void setGblTrackCollectionName (String val) {
        gblTrackCollectionName = val;
    }
    public void setDebugLevel(int val) { 
        debugLevel = val;
    }

    public void setFullyUnbiased(boolean val) { 
        fullyUnbiased = val;
    }
    ...
    public void detectorChanged(Detector detector){
        ...
        for(HpsSiSensor sensor:sensors){
            ...
            b_residualY.put(sensorName,histogramFactory.createHistogram1D("b_res_" + sensorName, nBins, b_minYRes, b_maxYRes));
            ...
            residualYvsCluSize.put(sensorName,histogramFactory.createHistogram2D("u_res_vs_cluSize_"+sensorName,5,0,5,nBins,minYRes,maxYRes));
            b_residualYvsCluSize.put(sensorName,histogramFactory.createHistogram2D("u_res_vs_cluSize_"+sensorName,5,0,5,nBins,b_minYRes,b_maxYRes));
            
            residualYvsP.put(sensorName,histogramFactory.createHistogram2D("u_res_vs_p_" + sensorName, 25, 0, 5, nBins, minYRes, maxYRes));
            b_residualYvsP.put(sensorName,histogramFactory.createHistogram2D("b_res_vs_p_" + sensorName, 25, 0, 5, nBins, b_minYRes, b_maxYRes));
    ...
    //Return the HpsSiSensor for a given Volume and MPID - TODO Place this in an util class
    private HpsSiSensor getSensor(boolean IsTop, int milleID) {
        
        //gblScatterSensor is a reference of one of the sensors
        HpsSiSensor gblScatterSensor = null;
        for (HpsSiSensor sensor : sensors) {
            if (sensor.getMillepedeId() == milleID && sensor.isTopLayer() == IsTop) {
                gblScatterSensor = sensor;
                break;
            }
        }
        return gblScatterSensor;
    }
    ...
    //TODO Remove the gblSC, it's not needed
    private GBLStripClusterData getOtherSideGblSCHit(List<GBLStripClusterData> gblStripClustersOnTrack, GBLStripClusterData gblSC, HpsSiSensor gblScatterSensor) {
        
        //Informations of the hit that is being removed
        int hit_mpid     = gblSC.getId();
        int i_layer      = ((gblScatterSensor.getLayerNumber() + 1) / 2) - 1;
        boolean isAxial  = gblScatterSensor.isAxial();
        String side      = gblScatterSensor.getSide();
        boolean IsTop    = gblScatterSensor.isTopLayer();
        
        GBLStripClusterData gblSC_to_return = null;
        
        for (GBLStripClusterData gblSC_otherSide : gblStripClustersOnTrack) {
            
            //The other side element to remove needs to have:
            //Same layer, same side, same bottom/top, axial/stereo opposite.
            //And check if you have a hit there! => TODO - only needed for 2D hits
            //get the layer from the sensor name?
            
            //Millipede ID needs to be +/- 1 wrt sensor I want to remove
            if (abs(gblSC_otherSide.getId() - hit_mpid) != 1)
                continue;
            
            //Get the HpsSiSensor info corresponding to this gblSC_for_refit
            HpsSiSensor otherSide_gblScatterSensor = getSensor(IsTop,gblSC_otherSide.getId());
            
            //It has to be the same layer
            int otherSide_ly = ((otherSide_gblScatterSensor.getLayerNumber() + 1) / 2) - 1;
            
            if (otherSide_ly == i_layer) {
                //Check that axial and stereo are opposite
                if (otherSide_gblScatterSensor.isAxial() == isAxial) {
                    System.out.println("ERROR Removing other side layer. Both have the same strip orientation. Axial=" + isAxial);
                }
                
                //They have to have be on the same side (?)
                if (!((otherSide_gblScatterSensor.getSide()).equals(side))) {
                    System.out.println("ERROR Removing other side layer. They have different side: " + side + " otherSide: " + otherSide_gblScatterSensor.getSide());
                }
                
                //Remove it.
                if (debugLevel > 1) {
                    System.out.println("ORIGINAL SENSOR::");
                    System.out.println(gblScatterSensor.toString());
                    System.out.println("OTHER SIDE SENSOR::");
                    System.out.println(otherSide_gblScatterSensor.toString());
                }
                
                gblSC_to_return = gblSC_otherSide;
                break;                
            }
        }
        
        return gblSC_to_return;
    }
    /*
    private Pair<Track, FittedGblTrajectory> refitPoints(Track track, List<GBLStripClusterData> u_gblSCs, double bfield, int nIterations) {
        
        HelicalTrackFit newHelix = null;
        for (int Iter = 0 ; Iter<nIterations; Iter++) {
            
            double[] ref = new double[] {0.,0.,0.};
            FittedGblTrajectory refit = gbl_refitter.fit(u_gblSCs,bfac,false);
            Pair<double[], SymmetricMatrix> helixParams = refit.getCorrectedPerigeeParameters(TrackUtils.getHTF(track), FittedGblTrajectory.GBLPOINT.IP,bfield);
            
            //make the corrected track
            Trackstate tsIP = new BaseTrackState(helixParams.getFirst(), ref, 
                                                    helixParams.getSecond().asPackedArray(true),
                                                    TrackState.AtIP,bfield);
            
            HelicalTrackFit newHelix = TrackUtils.getHTF(tsIP);
            
            //Recompute multiple scattering angles and extrapolated positions
            MultipleScattering.ScatterPoints scatters = _scattering.FindHPSScatterPoints(newHelix);
    ...
    */
    ...
    private double computeRefitAndResiduals(Track track, GBLStripClusterData gblSC, List<GBLStripClusterData> gblStripClustersOnTrack, HpsSiSensor gblScatterSensor, boolean doBiasedResiduals) {
        ...
        //List<GBLStripClusterData> test = UnbiasedgblSCsForRefit(gblStripClustersOnTrack,gblSC,gblScatterSensor,true);
        ...
        //Biased: do not remove anything
        //Unbiased: remove just the side
        //Fully Unbiased: remove both side and other side

        //Get the other side
        GBLStripClusterData gblSC_otherSide = getOtherSideGblSCHit(gblStripClustersOnTrack,gblSC,gblScatterSensor);
        

        
        for (GBLStripClusterData gblSC_for_refit : gblStripClustersOnTrack) {
            
            //if biased re-add all hits
            if (doBiasedResiduals)
                u_gblSCs.add(gblSC_for_refit);
            else { // unbiased residuals
                
                //if fullyUnbiased remove hit and other side hit
                if (fullyUnbiased) {
                    
                    //if it's not an element to remove just add it
                    if ((gblSC_for_refit.getId() != gblSC.getId()) && gblSC_for_refit.getId() != gblSC_otherSide.getId())
                        u_gblSCs.add(gblSC_for_refit);
                    else {
                        
                        //Copy the right strip cluster, set it to scatter only and add it
                        ...
                        if (gblSC_for_refit.getId() != gblSC_otherSide.getId()) {
                            GBLStripClusterData gblSC_otherSide_copy = new GBLStripClusterData(gblSC_otherSide);
                            gblSC_otherSide_copy.setScatterOnly(1);
                            u_gblSCs.add(gblSC_otherSide_copy);
                        }
                        
                    }
                } 
                // if not fully Unbiased only remove this sensor
                else { 
                    if (gblSC_for_refit.getId() != gblSC.getId())
                        u_gblSCs.add(gblSC_for_refit);
                    else {
            ...
            }// do unbiased residuals
                
        }//compute list of points to fit
