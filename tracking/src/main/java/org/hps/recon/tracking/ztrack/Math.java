...
import org.hps.recon.tracking.lit.*;
...
public class Math {
    ...
    public static double ChiSq(
            ...
            Hit hit) {
        double chisq = 0.;
        if (hit.GetType() == HitType.STRIPHIT) {
            if (hit instanceof DetPlaneStripHit) {
                chisq = ChiSq(par, (DetPlaneStripHit) hit);
            } else {
                chisq = ChiSq(par, (StripHit) hit);
            }
        } else if (hit.GetType() == HitType.PIXELHIT) {
            chisq = ChiSq(par, (PixelHit) hit);
        }
        return chisq;
    }
    ...
    public static int NDF(
        ...
        for (int i = 0; i < track.GetNofHits(); i++) {
            if (track.GetHit(i).GetType() == HitType.PIXELHIT) {
                ndf += 2;
            } else if (track.GetHit(i).GetType() == HitType.STRIPHIT) {
                ndf++;
            }
        }
        ndf -= 5;
