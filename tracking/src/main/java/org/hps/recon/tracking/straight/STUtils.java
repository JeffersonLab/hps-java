...
public class STUtils {
    ...
    abstract static class STBaseTrack {
        ...
        protected void checkFit() {
            if (getFit() == null)
                throw new RuntimeException("This track has no fit");
        }
    ...
    static class STTrack extends STBaseTrack {
        ...
        public boolean isTop() {
            if (hits.size() == 0)
                throw new RuntimeException("need hits to determine half.");
        ...
        public String toString() {
            ...
            for (SiTrackerHitStrip1D hit : hits) {
                s += hit.getPositionAsVector().toString() + "("
    ...
    static class STStereoTrack {
        ...
        void checkView(VIEW view) {
            if (!view.equals(VIEW.YZ) && !view.equals(VIEW.XZ))
                throw new RuntimeException("This view is not valid.");
        }
        ...
        /**
         * Get list of points that would be used in a simple univariate regression. Note that one {@link VIEW} uses the
         * path length.
         * 
        ...
        public String toString() {
            ...
            for (StereoPair pair : hits) {
                sb.append("Pair: pos " + pair.getPosition() + " "
                        + pair.getAxial().getRawHits().get(0).getDetectorElement().getName() + " org "
                        + getOrigin(pair.getAxial()).toString() + " "
                        + pair.getStereo().getRawHits().get(0).getDetectorElement().getName() + " org "
                        + getOrigin(pair.getStereo()).toString() + "\n");
            }
            sb.append("\nPointlists:\n");
            for (int i = 0; i < 2; ++i) {
    ...
    static class STTrackFit extends SlopeInterceptLineFit {
        ...
        public STTrackFit(String type, SlopeInterceptLineFit fit) {
            super(fit.slope(), fit.intercept(), fit.slopeUncertainty(), fit.interceptUncertainty(), fit.covariance(),
                    fit.chisquared(), fit.ndf());
            this.type = type;
        }

        public STTrackFit(String type, double slope, double intercept, double slopeUncertainty,
                double interceptUncertainty, double sigAB, double chisq, int ndf) {
    ...
    static class RegressionFit extends STTrackFitter {
        ...
        public void fit(List<double[]> xyList) {
            ...
            setFit(new STTrackFit(TYPE, regression.getSlope(), regression.getIntercept(), regression.getSlopeStdErr(),
                    regression.getInterceptStdErr(), regression.getR(), 0.0, (int) (regression.getN() - 2)));
        }
     ...
     */
    static Hep3Vector getStereoHitPositionFromReference(Hep3Vector origin, SiTrackerHitStrip1D strip1,
            SiTrackerHitStrip1D strip2) {
        SiTrackerHitStrip1D s1 = strip1;
        SiTrackerHitStrip1D s2 = strip2;
        // sort in direction of track
        // TODO do I need to sort these?
        if (s1.getPositionAsVector().z() > s2.getPositionAsVector().z()) {
            s1 = strip2;
            s2 = strip1;
        }

        StraightThroughAnalysisDriver.logger.finest("Calculate stereo hit position assuming track from "
                + origin.toString());
        ...
        // check
        if (Math.abs(VecOp.dot(o1, w1)) < 0.0001)
            throw new RuntimeException("o1 and w1 are orthogonal: o1 " + o1.toString() + " w1 " + w1.toString());
    ...
    static Hep3Vector getPosition(Hep3Vector t, SiTrackerHitStrip1D strip1, SiTrackerHitStrip1D strip2) {
        ...
        StraightThroughAnalysisDriver.logger.finest("Calculate stereo hit position assuming track direction "
                + t.toString());
    ...
    /**
     * Fit the {@link STUtils.STStereoTrack} track with the supplied {@link STUtils.STTrackFitter} and add the fit to
     * the track.
     * 
    ...
    public static double getTraversedMaterial(STStereoTrack track, SiTrackerHitStrip1D hit) {
        ...
        if ((track.getDirection().magnitude() - 1) > 0.0001)
            throw new RuntimeException("track dir is not normalized? " + track.getDirection().toString() + " mag = "
                    + track.getDirection().magnitude());
    ...
    public static void printGBL(PrintWriter printWriter, EventHeader event, List<STStereoTrack> tracks) {
        ...
        for (int iTrack = 0; iTrack < tracks.size(); ++iTrack) {
            ...
            printWriter.printf("Track perPar (y0 z0 dydx dzdx) %.12f %.12f %.12f %.12f", perPars[0], perPars[1],
                    perPars[2], perPars[3]);
            printWriter.println();

            double[] clPars = getCLPars(track);
            printWriter.printf("Track clPar (q/p lambda phi xT yT) %.12f %.12f %.12f %.12f %.12f", clPars[0],
                    clPars[1], clPars[2], clPars[3], clPars[4]);
            printWriter.println();

            Hep3Matrix clPrj = curvilinearProjectionMatrix(CoordinateTransformations.transformVectorToTracking(track
                    .getDirection()));
            ...
            for (StereoPair pair : track.getHits()) {
                ...
                for (int i = 0; i < strips.length; ++i) {
                    ...
                    printWriter
                            .printf("New Strip id layer %d %d %s", iStrip, sensor.getMillepedeId(), sensor.getName());
                    printWriter.println();
                    ...
                    Hep3Vector stereoHitPosition = CoordinateTransformations.transformVectorToTracking(pair
                            .getPosition());
                    printWriter.printf("Strip 3D hit pos %.12f %.12f %.12f", stereoHitPosition.x(),
                            stereoHitPosition.y(), stereoHitPosition.z());
                    printWriter.println();
                    ...
                    printWriter.printf("Strip track pos %.12f %.12f %.12f", trackPosition.x(), trackPosition.y(),
                            trackPosition.z());
                    printWriter.println();
                    ...
                    printWriter.printf("Strip u %.12f",
                            strip.getTransformedHit(CoordinateSystem.SENSOR).getPosition()[0]);
                    printWriter.println();

                    double uRes = getUResidual(strip, track);
                    double uRes_err = Math.sqrt(strip.getTransformedHit(CoordinateSystem.SENSOR)
                            .getCovarianceAsMatrix().diagonal(0));
    ...
    private static Hep3Vector getLinePlaneIntercept(SiTrackerHitStrip1D strip, STStereoTrack track) {
        // line description
        Hep3Vector l0 = new BasicHep3Vector(track.getIntercept()[VIEW.XZ.ordinal()],
                track.getIntercept()[VIEW.YZ.ordinal()], 0);
    ...
    protected static double getUResidual(SiTrackerHitStrip1D strip, STStereoTrack track) {
        ...
        logger.finest("u " + strip.getMeasuredCoordinate().toString() + " v "
                + strip.getUnmeasuredCoordinate().toString() + " w "
                + VecOp.cross(strip.getMeasuredCoordinate(), strip.getUnmeasuredCoordinate()).toString());
        ...
        logger.finest("\nures " + ures + " \ntrkpos " + trkpos.toString() + " \ntrkposOriginLocal "
                + trkposOriginLocal.toString() + " \nmeaspos " + measpos.toString() + " \nmeasposLocal "
                + measposLocal.toString());
    ...
    public static double[] getPerPars(STStereoTrack track) {
        Hep3Vector interceptGlobalFrame = new BasicHep3Vector(track.getIntercept(VIEW.XZ), track.getIntercept(VIEW.YZ),
                0);
        ...
        logger.finest("intercept global " + interceptGlobalFrame.toString() + " tracking "
                + interceptTrackingFrame.toString());
        logger.finest("slope global " + slopeGlobalFrame.toString() + " tracking " + slopeTrackingFrame.toString());
        return new double[] {interceptTrackingFrame.y(), interceptTrackingFrame.z(), slopeTrackingFrame.y(),
    ...
    public static double[] getCLPars(STStereoTrack track) {
        return getCLPars(track.getMomentum(), track.getIntercept(VIEW.XZ), track.getIntercept(VIEW.YZ),
                track.getSlope(VIEW.XZ), track.getSlope(VIEW.YZ));
    }
