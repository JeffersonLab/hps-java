 ...
 * @version $Id:
 */
public class HpsLitTrack {

    TreeSet<HpsStripHit> fHits; // Array of hits
    List<CbmLitFitNode> fFitNodes; // Array of fit nodes
    CbmLitTrackParam fParamFirst = new CbmLitTrackParam(); // First track parameter
    CbmLitTrackParam fParamLast = new CbmLitTrackParam(); // Last track parameter
    LitTrackQa fQuality; // Track quality
    double fChi2; // Chi-square
    int fNDF; // Number of degrees of freedom
    int fPDG; // particle hypothesis
    double fLength;

    public HpsLitTrack() {
        fHits = new TreeSet<HpsStripHit>();
...
//
    ...
    public Set< HpsStripHit> GetHits() {
        return fHits;
    }

    public Set<HpsStripHit> GetReverseHits() {
        return fHits.descendingSet();
    }
//
...
//
     ...
     */
    public void AddHit(HpsStripHit hit) {
        fHits.add(hit);
    }
//
