    ...
    public void processSim(List<MCParticle> mcParticles, List<SimTrackerHit> simTrackerHits) {
        ...
        for (SimTrackerHit sh : simTrackerHits) {
            ...
            if (Math.abs(shpos.x()) < 50.0) {
                System.out.printf("%s: Weird hit at %s (%s) in layer %d for MC part %d org %s p %s\n", this.getClass().getSimpleName(), sh.getPositionVec().toString(), shpos.toString(), sh.getIdentifierFieldValue("layer"), sh.getMCParticle().getPDGID(), sh.getMCParticle().getOrigin().toString(), sh.getMCParticle().getMomentum().toString());
                System.exit(1);
            }
        ...
        for (int layer = 1; layer < 13; ++layer) {
            ...
            if (simHitsLayer != null) {
                ...
                for (SimTrackerHit simHit : simHitsLayer) {
                    ...
                    if (mcp.getMomentum().magnitude() < 0.5)
                        continue;
                    ...
                    if (_debug > 0) {
                        System.out.printf("%s: simHit for layer %d at %s (%s) from MC part %d org %s p %s\n", this.getClass().getSimpleName(), layer, simHit.getPositionVec().toString(), simHitPosTracking.toString(), simHit.getMCParticle().getPDGID(), simHit.getMCParticle().getOrigin().toString(), simHit.getMCParticle().getMomentum().toString());
