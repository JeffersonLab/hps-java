...
class Helix { // Create a simple helix oriented along the B field axis for testing the Kalman
    ...
    private RotMatrix R; // Rotation from global coordinates to the B field frame
    private Vec origin; // Origin of the B-field reference frame in global coordinates (where B field
                        // map is evaluated)

    private FieldMap fM;

    Helix(Vec HelixParams, Vec pivot, Vec origin, FieldMap fM) {
        this.origin = origin.copy();
        this.fM = fM;
        p = HelixParams.copy();
        gran = new double[2];
        Vec Bf = fM.getField(origin);
        B = Bf.mag();
        double c = 2.99793e8;
    ...
    Helix copy() {
        return new Helix(p, X0, origin, fM);
    }
    ...
    double planeIntersect(Plane Pin) { // phi value where the helix intersects the plane P (given in global
        ...
        // P.print("local from helix.planeIntersect");
        // Pin.print("global from helix.planeIntersect");
        // p.print("helix from helix.planeIntersect");
        ...
        if (Double.isNaN(phi0) || P.T().v[2] == 1.0)
            return phi0;
        ...
        // System.out.format("Helix.planeIntersect: phi0=%12.10f, phi=%12.10f\n", phi0,
        // phi);
        return phi;
    }
    ...
    Helix randomScat(Plane P, double X) { // Produce a new helix scattered randomly in a given plane P
        ...
        if (X == 0.)
            ...
            theta0 = Math.sqrt(2.0 * (X / radLen) / ct) * (0.0136 / pmom.mag()) * (1.0 + 0.038 * Math.log((X / radLen) / ct));
        double thetaScat = gausRan() * theta0;
        double phiScat = Math.random() * 2.0 * Math.PI;
        // System.out.format("randomScat: theta0=%10.6f thetaScat=%10.7f
        ...
        // System.out.format("uhat dot t=%12.8f\n", uhat.dot(t)); //Crosscheck that uhat
        // is still perpendicular to t
        RotMatrix R2;
        if (thetaScat == 0.)
            R2 = new RotMatrix(0.);
        else
            R2 = new RotMatrix(uhat, thetaScat); // Now rotate the helix direction by theta about the u axis
        ...
        // R2.print("scattering R2");
        // Rnew.print("Rnew");
        Vec tnew = Rnew.rotate(R2.rotate(t));
        // R2.rotate(t).print("scattered direction in global coordinates");
        // System.out.format("tnew dot tnew= %14.10f\n", tnew.dot(tnew));
        // System.out.format("randomScat: new direction in B-field coordinates=%10.7f,
        ...
        /*
         * // Testing of the pivot transformation equations: double [] aP =
         * pivotTransform(r.v, p, B); System.out.format("Comparing transformations:\n");
         * for (int i=0; i<5; i++) { System.out.format("     %d   %10.6f    %10.6f\n",
         * i, H[i], aP[i]); }
         * 
         * Matrix5 F = makeF(aP, p, B); // analytic derivatives
         * 
         * double [] dP = new double [5]; double [] ppdP = new double [5]; for (int i=0;
         * i<5; i++) { dP[i] = p[i]*0.1; ppdP[i] = p[i] + dP[i]; } double [] aPpdP =
         * pivotTransform(r.v, ppdP, B); double [] N = new double[5]; // numerical
         * estimates from derivatives for (int i=0; i<5; i++) { N[i] = aP[i]; for (int
         * j=0; j<5; j++) { N[i] = N[i] + F.M[i][j] * dP[j]; } }
         * System.out.format("Comparing with numerical difference:\n"); for (int i=0;
         * i<5; i++) { System.out.format("     %d   %10.6f    %10.6f\n", i, aPpdP[i],
         * N[i]); }
         */
        return new Helix(H, new Vec(0., 0., 0.), r, fM); // Create the new helix with pivot point specified in the new
                                                         // B-field coordinates
    }
