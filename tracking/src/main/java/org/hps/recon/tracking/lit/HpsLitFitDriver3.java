 ...
 */
public class HpsLitFitDriver3 extends Driver {
    ...
    private Map<String, CbmLitDetPlaneStripHit> planeHits = new HashMap<String, CbmLitDetPlaneStripHit>();
...
//        setupTransforms(detector);
    ...
    protected void process(EventHeader event) {
        ...
        MCParticle.SimulatorStatus simstat = mcParticles.get(0).getSimulatorStatus();
...
//                Hep3Vector[] r = new Hep3Vector[12]; // translation vector local to global
                    ...
                    for (TrackerHit h : hits) { // these are 3D HelicalTrackHits
                        Set<TrackerHit> stripList = hitToStrips.allFrom(hitToRotated.from(h));
                        for (TrackerHit strip : stripList) { // these are 1D Strip clusters
                            ...
                            planeHits.put(sensor.getName(), trackHit);
                        }
                    ...
                    for (MCParticle mcp : mcParticles) {
                        if (mcp.getGeneratorStatus() == MCParticle.FINAL_STATE) {
                            ...
                            for (CbmLitDetPlaneStripHit hit : trackHitList) {
                                ...
                                // let's compare this tothe SimTrackerHit at this sensor
                                Hep3Vector mcPosGlobal = new BasicHep3Vector(simTrackerHitmap.get(name).getPosition());
                                Hep3Vector mcPosLocal = dp.toLocal(mcPosGlobal);
                                System.out.println("SimTrackerHit global " + mcPosGlobal);
                                System.out.println("SimTrackerHit local " + mcPosLocal);

                                // let's create a strip hit and use this as our first track fit
                                ...
                                //update our track state for the next iteration...
                            ...
                            CbmLitTrack hpsTrack = fit(trackHitList);
                            compare(hpsTrack, parIn, "hpsTrack");

                            CbmLitTrack mcPropTrack = fit(mcPropagatedDetPlaneHits);
                            compare(mcPropTrack, parIn, "mcPropTrack");
...
//            }
//        }
        }// end of requiring one good MCParticle and one track
    }
    ...
    private void compare(CbmLitTrack track, CbmLitTrackParam mcp, String folder) {
        ...
        double[] minVal = {-1.0, -0.5, -0.02, -0.001, -1.};// x,y,tx,ty,q/p
        double[] maxVal = {1.0, 0.5, 0.02, 0.001, 1.};

        for (int i = 0; i < 5; ++i) {
            aida.histogram1D(label[i] + " MC", 100, minVal[i], maxVal[i]).fill(mcStateVector[i]);
            aida.histogram1D(label[i] + " track", 100, minVal[i], maxVal[i]).fill(tStateVector[i]);
        ...
        aida.histogram1D("Momentum", 100, 2.0, 3.0).fill(abs(1. / tStateVector[4]));
        _tree.cd("/");
    }
