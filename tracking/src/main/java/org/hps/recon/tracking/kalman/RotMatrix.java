...
class RotMatrix {  // 3 by 3 rotation matrix for the Kalman filter
	...
	RotMatrix(Vec u, Vec v, Vec t) {  // Transforms from global coordinates to frame with 3-D unit vectors u, v, t
		M = new double [3][3];
		for (int i=0; i<3; i++) {
			M[0][i] = u.v[i]; M[1][i] = v.v[i]; M[2][i] = t.v[i];
		}
	}
	...
	Vec inverseRotate(Vec V) {   // Use the matrix to rotate a 3-D vector in the opposite sense, using the inverse (i.e. transpose) of the rotation matrix
		Vec Vp = new Vec(0.,0.,0.);
		for (int i=0; i<3; i++) {
			for (int j=0; j<3; j++) {
				Vp.v[i] += M[j][i]*V.v[j];
