...
public class StraightThroughAnalysisDriver extends Driver {
    ...
    protected void detectorChanged(Detector detector) {
        ...
        plotters.get("Sensor stereo YZ hit res").setStyle(
                this.getDefaultPlotterStyle("Stereo hit y res global (mm)", "Entries"));
        ...
        plotters.get("Sensor stereo XZ hit res").setStyle(
                this.getDefaultPlotterStyle("Stereo hit x res global (mm)", "Entries"));
        ...
        plotters.get("Fit update iterations").setStyle(
                this.getDefaultPlotterStyle("Iterations", "Average hit update magnitude (mm)"));
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            trackExtraPolationY[i] = hf.createHistogram2D("Track " + half + " extrapolation Y", nPointsZ, endPointZ,
                    startPointZ, 50, -80, 80);
            trackExtraPolationX[i] = hf.createHistogram2D("Track " + half + " extrapolation X", nPointsZ, endPointZ,
                    startPointZ, 50, -100, 60);
            ...
            plotters.put("Track L13 " + half + " axial",
                    af.createPlotterFactory().create("Track L13 " + half + " axial"));
            // plotters.get("Track " + half +" axial").setStyle(this.getDefaultPlotterStyle("","Entries",true));
            ...
            plotters.put("Track L46 " + half + " axial",
                    af.createPlotterFactory().create("Track L46 " + half + " axial"));
            // plotters.get("Track " + half +" axial").setStyle(this.getDefaultPlotterStyle("","Entries",true));
            ...
            plotters.put("Track L13L46 " + half + " axial",
                    af.createPlotterFactory().create("Track L13L46 " + half + " axial"));
            // plotters.get("Track " + half +" axial").setStyle(this.getDefaultPlotterStyle("","Entries",true));
        ...
        trackAxialExtraPolation[2] = hf.createHistogram2D("Track axial extrapolation", nPointsZ, endPointZ,
                startPointZ, 50, -80, 80);
        plotters.get("Track axial extrapolation").region(2).plot(trackAxialExtraPolation[2]);
        trackAxialExtraPolation[3] = hf.createHistogram2D("Track axial extrapolation wires", nPointsZ,
                wirePosition[2] - 350, wirePosition[2] + 350, 50, -20, 20);
        plotters.get("Track axial extrapolation").region(3).plot(trackAxialExtraPolation[3]);
        trackAxialExtraPolation[4] = hf.createHistogram2D("Track axial extrapolation rndm", nPointsZ, -200.0, 200.0,
                50, -60, 60);
        plotters.get("Track axial extrapolation").region(4).plot(trackAxialExtraPolation[4]);

        trackExtraPolationY[2] = hf.createHistogram2D("Track extrapolation Y", nPointsZ, endPointZ, startPointZ, 50,
                -80, 80);
        plotters.get("Track extrapolation Y").region(2).plot(trackExtraPolationY[2]);
        trackExtraPolationY[3] = hf.createHistogram2D("Track extrapolation Y wires", nPointsZ, wirePosition[2] - 350,
                wirePosition[2] + 350, 50, -20, 20);
        plotters.get("Track extrapolation Y").region(3).plot(trackExtraPolationY[3]);
        trackExtraPolationY[4] = hf.createHistogram2D("Track extrapolation Y rndm", nPointsZ, -200.0, 200.0, 50, -60,
                60);
        plotters.get("Track extrapolation Y").region(4).plot(trackExtraPolationY[4]);

        trackExtraPolationX[2] = hf.createHistogram2D("Track extrapolation X", nPointsZ, endPointZ, startPointZ, 50,
                -100, 60);
        plotters.get("Track extrapolation X").region(2).plot(trackExtraPolationX[2]);
        trackExtraPolationX[3] = hf.createHistogram2D("Track extrapolation X wires", nPointsZ, wirePosition[2] - 350,
                wirePosition[2] + 350, 50, -80, -20);
        plotters.get("Track extrapolation X").region(3).plot(trackExtraPolationX[3]);
        trackExtraPolationX[4] = hf
                .createHistogram2D("Track extrapolation X rndm", nPointsZ, -200.0, 200.0, 50, -60, 0);
        ...
        for (HpsSiSensor sensor : sensors) {
            ...
            if (sensor.isBottomLayer()) {
                sensorHitPositions.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_hitpos1D", 50, -60, 0));
                if (sensor.isAxial()) {
                    stereoHitPositionsXY.put(sensor.getName(),
                            hf.createHistogram2D(sensor.getName() + "_stereohitpos2D", 50, -100, 1000, 50, -60, 0));
                    plotters.get("Stereo hit position").region(SvtPlotUtils.computePlotterRegionAxialOnly(sensor))
                            .plot(stereoHitPositionsXY.get(sensor.getName()));
                }
            } else {
                sensorHitPositions.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_hitpos1D", 50, 0, 60));
                if (sensor.isAxial()) {
                    stereoHitPositionsXY.put(sensor.getName(),
                            hf.createHistogram2D(sensor.getName() + "_stereohitpos2D", 50, -100, 100, 50, 0, 60));
                    plotters.get("Stereo hit position").region(SvtPlotUtils.computePlotterRegionAxialOnly(sensor))
                            .plot(stereoHitPositionsXY.get(sensor.getName()));
                }
            }
            ...
            plotters.get("Sensor u res").region(SvtPlotUtils.computePlotterRegion(sensor))
                    .plot(sensorURes.get(sensor.getName()));
            plotters.get("Sensor hit position").region(SvtPlotUtils.computePlotterRegion(sensor))
                    .plot(sensorHitPositions.get(sensor.getName()));
            plotters.get("Sensor hit times").region(SvtPlotUtils.computePlotterRegion(sensor))
                    .plot(sensorHitTimes.get(sensor.getName()));
            plotters.get("Sensor cluster counts").region(SvtPlotUtils.computePlotterRegion(sensor))
                    .plot(sensorHitCounts.get(sensor.getName()));
            ...
            if (sensor.isAxial()) {
                ...
                sensorHitResGlobalL13.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_hitresglobalL13", 50, -0.7, 0.7));
                ...
                sensorHitResGlobalL46.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_hitresglobalL46", 50, -0.7, 0.7));
                ...
                sensorStereoHitYZResGlobal.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_stereohityzresglobal", 50, -1, 1));
                plotters.get("Sensor stereo YZ hit res").region(SvtPlotUtils.computePlotterRegionAxialOnly(sensor))
                        .plot(sensorStereoHitYZResGlobal.get(sensor.getName()));
                plotters.get("Sensor stereo YZ hit res").region(0).style().dataStyle().showInStatisticsBox(true);
                sensorStereoHitXZResGlobal.put(sensor.getName(),
                        hf.createHistogram1D(sensor.getName() + "_stereohitxzresglobal", 50, -1, 1));
                plotters.get("Sensor stereo XZ hit res").region(SvtPlotUtils.computePlotterRegionAxialOnly(sensor))
                        .plot(sensorStereoHitXZResGlobal.get(sensor.getName()));
    ...
    protected void process(EventHeader event) {
            ...
            for (SiTrackerHitStrip1D axialSeedHit : aSeedHits) {
                ...
                for (List<SiTrackerHitStrip1D> sSeedHits : stereoSeedHits) {
                    for (SiTrackerHitStrip1D stereoSeedHitCandidate : sSeedHits) {
                        HpsSiSensor sensor = (HpsSiSensor) stereoSeedHitCandidate.getRawHits().get(0)
                                .getDetectorElement();
                        if (sensor.getName().equals(stereoSensor.getName())) {
                            logger.fine("Found stereo hit at "
                                    + stereoSeedHitCandidate.getPositionAsVector().toString() + " at sensor \""
                                    + stereoSensor.getName() + "\"");
        ...
        for (int ihalf = 0; ihalf < 2; ++ihalf) {
            ...
            for (STUtils.StereoPair pair : pairs) {
                ...
                stereoHitPositionsXY.get(pair.getAxial().getRawHits().get(0).getDetectorElement().getName()).fill(
                        p.x(), p.y());
                logger.fine(p.toString() + " from " + pair.getAxial().getPositionAsVector().toString() + "  "
                        + pair.getStereo().getPositionAsVector().toString() + " ("
                        + pair.getAxial().getRawHits().get(0).getDetectorElement().getName() + " and "
                        + pair.getStereo().getRawHits().get(0).getDetectorElement().getName() + ")");
            }
        }
        ...
        for (STUtils.STStereoTrack track : stereoTracks) {
            ...
            for (STUtils.StereoPair hit : track.getHits()) {
                ...
                sensorStereoHitYZResGlobal.get(hit.getAxial().getRawHits().get(0).getDetectorElement().getName()).fill(
                        resGlobalY);
                sensorStereoHitXZResGlobal.get(hit.getAxial().getRawHits().get(0).getDetectorElement().getName()).fill(
                        resGlobalX);
        ...
        for (HpsSiSensor sensor : sensors) {
            ...
            if (h != null) {
                if (h.entries() > 20) {
                    ...
                    if (f != null) {
                        SvtPlotUtils.plot(plotters.get("Sensor hit res"), f.fittedFunction(), null,
                                SvtPlotUtils.computePlotterRegionAxialOnly(sensor));
                    }
                }
            }
        }
