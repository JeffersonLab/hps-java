 ...
 * comments TODO Move general functions to a different driver TODO Fix u error
 * function TODO Update Track Extrapolations
 */
public class SvtHitEfficiencyKalman extends Driver {
	...
	public void detectorChanged(Detector detector) {
		...
		for (HpsSiSensor sensor : sensors) {
			...
			double maxV = width / 2;
			double minV = -maxV;
			numberOfTracksChannel.put(sensorName, histogramFactory.createHistogram1D(
					"Number of Tracks Channel " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksWithHitOnMissingLayerChannel.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks With Hit Channel " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannel.put(sensorName, histogramFactory.createHistogram1D(
					"HitEfficiency Channel " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksU.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks U " + sensorName, nBins, -maxU, maxU));
			numberOfTracksWithHitOnMissingLayerU.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit U " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyU.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U " + sensorName, nBins, -maxU, maxU));
			numberOfTracksP.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks P " + sensorName, nBins, 0, 1.3 * ebeam));
			numberOfTracksWithHitOnMissingLayerP.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit P " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyP.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency P " + sensorName, nBins, 0, 1.3 * ebeam));

			numberOfTracksChannelEle.put(sensorName, histogramFactory.createHistogram1D(
					"Number of Tracks Channel Ele " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksWithHitOnMissingLayerChannelEle.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks With Hit Channel Ele " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannelEle.put(sensorName, histogramFactory.createHistogram1D(
					"HitEfficiency Channel Ele " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksUEle.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks U Ele " + sensorName, nBins, -maxU, maxU));
			numberOfTracksWithHitOnMissingLayerUEle.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit U Ele " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyUEle.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Ele " + sensorName, nBins, -maxU, maxU));
			numberOfTracksPEle.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks P Ele " + sensorName, nBins, 0, 1.3 * ebeam));
			numberOfTracksWithHitOnMissingLayerPEle.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit P Ele " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyPEle.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency P Ele " + sensorName, nBins, 0, 1.3 * ebeam));

			numberOfTracksChannelPos.put(sensorName, histogramFactory.createHistogram1D(
					"Number of Tracks Channel Pos " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksWithHitOnMissingLayerChannelPos.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks With Hit Channel Pos " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannelPos.put(sensorName, histogramFactory.createHistogram1D(
					"HitEfficiency Channel Pos " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksUPos.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks U Pos " + sensorName, nBins, -maxU, maxU));
			numberOfTracksWithHitOnMissingLayerUPos.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit U Pos " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyUPos.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Pos " + sensorName, nBins, -maxU, maxU));
			numberOfTracksPPos.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks P Pos " + sensorName, nBins, 0, 1.3 * ebeam));
			numberOfTracksWithHitOnMissingLayerPPos.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks With Hit P Pos " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyPPos.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency P Pos " + sensorName, nBins, 0, 1.3 * ebeam));

			numberOfTracksChannelCorrected.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks Channel Corrected " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannelCorrected.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksUCorrected.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks U Corrected " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyUCorrected.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Corrected " + sensorName, nBins, -maxU, maxU));
			numberOfTracksPCorrected.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks P Corrected " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyPCorrected.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected " + sensorName, nBins, 0, 1.3 * ebeam));

			numberOfTracksChannelCorrectedEle.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks Channel Corrected Ele " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannelCorrectedEle.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected Ele " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksUCorrectedEle.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks U Corrected Ele " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyUCorrectedEle.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency U Corrected Ele " + sensorName, nBins, -maxU, maxU));
			numberOfTracksPCorrectedEle.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks P Corrected Ele " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyPCorrectedEle.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected Ele " + sensorName, nBins, 0, 1.3 * ebeam));

			numberOfTracksChannelCorrectedPos.put(sensorName,
					histogramFactory.createHistogram1D("Number of Tracks Channel Corrected Pos " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyChannelCorrectedPos.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected Pos " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			numberOfTracksUCorrectedPos.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks U Corrected Pos " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyUCorrectedPos.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency U Corrected Pos " + sensorName, nBins, -maxU, maxU));
			numberOfTracksPCorrectedPos.put(sensorName, histogramFactory
					.createHistogram1D("Number of Tracks P Corrected Pos " + sensorName, nBins, 0, 1.3 * ebeam));
			hitEfficiencyPCorrectedPos.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected Pos " + sensorName, nBins, 0, 1.3 * ebeam));
			...
			TotalCorrectedEff.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff " + sensorName, 1, 0, 1));
			TotalCorrectedEffEle.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff Ele " + sensorName, 1, 0, 1));
			TotalCorrectedEffPos.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff Pos " + sensorName, 1, 0, 1));

			hitEfficiencyChannelerr.put(sensorName, histogramFactory.createHistogram1D(
					"HitEfficiency Channel Error " + sensorName, nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyUerr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPerr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency P Error " + sensorName, nBins, 0, 1.3 * ebeam));

			hitEfficiencyChannelCorrectederr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected Error " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyUCorrectederr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency U Corrected Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPCorrectederr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected Error " + sensorName, nBins, 0, 1.3 * ebeam));

			hitEfficiencyChannelEleerr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Ele Error " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyUEleerr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Ele Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPEleerr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Ele Error " + sensorName, nBins, 0, 1.3 * ebeam));

			hitEfficiencyChannelCorrectedEleerr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected Ele Error " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyUCorrectedEleerr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency U Corrected Ele Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPCorrectedEleerr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected Ele Error " + sensorName, nBins, 0, 1.3 * ebeam));

			hitEfficiencyChannelPoserr.put(sensorName, histogramFactory.createHistogram1D(
					"HitEfficiency Channel Pos Error " + sensorName, nChan, -chanExtd, nChan + chanExtd));
			hitEfficiencyUPoserr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency U Pos Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPPoserr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Pos Error " + sensorName, nBins, 0, 1.3 * ebeam));

			hitEfficiencyChannelCorrectedPoserr.put(sensorName,
					histogramFactory.createHistogram1D("HitEfficiency Channel Corrected Pos Error " + sensorName,
							nChan + 2 * chanExtd, -chanExtd, nChan + chanExtd));
			hitEfficiencyUCorrectedPoserr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency U Corrected Pos Error " + sensorName, nBins, -maxU, maxU));
			hitEfficiencyPCorrectedPoserr.put(sensorName, histogramFactory
					.createHistogram1D("HitEfficiency P Corrected Pos Error " + sensorName, nBins, 0, 1.3 * ebeam));

			TotalEfferr.put(sensorName, histogramFactory.createHistogram1D("Total Eff Error " + sensorName, 1, 0, 1));
			TotalEffEleerr.put(sensorName,
					histogramFactory.createHistogram1D("Total Eff Ele Error " + sensorName, 1, 0, 1));
			TotalEffPoserr.put(sensorName,
					histogramFactory.createHistogram1D("Total Eff Pos Error " + sensorName, 1, 0, 1));

			TotalCorrectedEfferr.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff Error " + sensorName, 1, 0, 1));
			TotalCorrectedEffEleerr.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff Ele Error " + sensorName, 1, 0, 1));
			TotalCorrectedEffPoserr.put(sensorName,
					histogramFactory.createHistogram1D("Total Corrected Eff Pos Error " + sensorName, 1, 0, 1));

			errorU.put(sensorName, histogramFactory.createHistogram1D("Error U " + sensorName, nBins, 0, maxYerror));
			errorUvsV.put(sensorName, histogramFactory.createHistogram2D("Error U vs V " + sensorName, 2 * nBins, minV,
					maxV, nBins, 0, maxYerror));
			errorUvsU.put(sensorName, histogramFactory.createHistogram2D("Error U vs U " + sensorName, 2 * nBins, -maxU,
					maxU, nBins, 0, maxYerror));
			errorUvsP.put(sensorName, histogramFactory.createHistogram2D("Error U vs P " + sensorName, nBins, 0,
					1.3 * ebeam, nBins, 0, maxYerror));

			errorUEle.put(sensorName,
					histogramFactory.createHistogram1D("Error U Electrons " + sensorName, nBins, 0, maxYerror));
			errorUvsVEle.put(sensorName, histogramFactory.createHistogram2D("Error U vs V Electrons " + sensorName,
					2 * nBins, minV, maxV, nBins, 0, maxYerror));
			errorUvsUEle.put(sensorName, histogramFactory.createHistogram2D("Error U vs U Electrons " + sensorName,
					2 * nBins, -maxU, maxU, nBins, 0, maxYerror));
			errorUvsPEle.put(sensorName, histogramFactory.createHistogram2D("Error U vs P Electrons " + sensorName,
					nBins, 0, 1.3 * ebeam, nBins, 0, maxYerror));

			errorUPos.put(sensorName,
					histogramFactory.createHistogram1D("Error U Electrons " + sensorName, nBins, 0, maxYerror));
			errorUvsVPos.put(sensorName, histogramFactory.createHistogram2D("Error U vs V Positrons " + sensorName,
					2 * nBins, minV, maxV, nBins, 0, maxYerror));
			errorUvsUPos.put(sensorName, histogramFactory.createHistogram2D("Error U vs U Positrons " + sensorName,
					2 * nBins, -maxU, maxU, nBins, 0, maxYerror));
			errorUvsPPos.put(sensorName, histogramFactory.createHistogram2D("Error U vs P Positrons " + sensorName,
					nBins, 0, 1.3 * ebeam, nBins, 0, maxYerror));
			...
			Tanlambda.put(sensorName,
					histogramFactory.createHistogram1D("TanLambda " + sensorName, nBins, minTLambda, maxTLambda));
			...
			D0_err.put(sensorName,
					histogramFactory.createHistogram1D("D0 Error " + sensorName, nBins, minD0Err, maxD0Err));
			Z0_err.put(sensorName,
					histogramFactory.createHistogram1D("Z0 Error " + sensorName, nBins, minZ0Err, maxZ0Err));
			Tanlambda_err.put(sensorName, histogramFactory.createHistogram1D("TanLambda Error " + sensorName, nBins,
					minTLambdaErr, maxTLambdaErr));
			Phi0_err.put(sensorName,
					histogramFactory.createHistogram1D("Phi0 Error " + sensorName, nBins, minPhi0Err, maxPhi0Err));
			Omega_err.put(sensorName,
					histogramFactory.createHistogram1D("Omega Error " + sensorName, nBins, minOmegaErr, maxOmegaErr));
		}
	}
	...
	public void process(EventHeader event) {
		...
		List<LCRelation> kalTracksRelations = event.get(LCRelation.class, kalTrackRelationName);
		...
		for (Track track : tracks) {
            KalTrack kalTrk = null;
			for (LCRelation kalRel : kalTracksRelations) {
				if (kalRel.getFrom() == track)
					kalTrk = (KalTrack) kalRel.getTo();
			}
			if (kalTrk == null) {
				System.out.println("Couldn't find KalTrack to go with the track???");
				continue;
			}
			for (int l = 0; l < KalmanParams.numLayers; l++) {
				MeasurementSite site = kalTrk.lyrMap.get(l);
				boolean lyrHasHit = (site.hitID > -1);
				Pair<Double[], Double> intPair = kalTrk.unbiasedIntersect(site, true);
				Double[] intersect = intPair.getFirst();
				Double intError = Math.sqrt(intPair.getSecond());
				double[] originHelix = kalTrk.originHelixParms();
				double[][] originCov = kalTrk.originCovariance();

				// Fill track states and errors at IP
				D0.get(atIP).fill(originHelix[0]);
				Phi0.get(atIP).fill(originHelix[1]);
				Omega.get(atIP).fill(originHelix[2]);
				Z0.get(atIP).fill(originHelix[2]);
				Tanlambda.get(atIP).fill(originHelix[2]);
...
//			Tanlambda_err.get(atIP)
//					.fill(Math.sqrt(LocCovAtIP.e(HelicalTrackFit.slopeIndex, HelicalTrackFit.slopeIndex)));
//			Phi0_err.get(atIP).fill(Math.sqrt(LocCovAtIP.e(HelicalTrackFit.phi0Index, HelicalTrackFit.phi0Index)));
//			Omega_err.get(atIP)
//					.fill(Math.sqrt(LocCovAtIP.e(HelicalTrackFit.curvatureIndex, HelicalTrackFit.curvatureIndex)));

				Hep3Vector p = toHep3(kalTrk.originP());
				...
				// See if track is within acceptance of both the axial and stereo sensors of the
				// unused layer
				Pair<HpsSiSensor, Pair<Integer, Boolean>> sensorChanAccPair = getSensorChanInAcceptance(track,
						intersect, l);

				// Set axial and stereo sensors of the missing layer
				HpsSiSensor sensor = sensorChanAccPair.getFirst();

				String sensorName = sensor.getName();

				// Compute the channel where the track extrapolates to in each sensor
				int chan = sensorChanAccPair.getSecond().getFirst();
				Boolean inAcceptance = sensorChanAccPair.getSecond().getSecond();

				double trackP = toHep3(track.getTrackStates().get(0).getMomentum()).magnitude();

				double weight = findWeight(intersect[0], intError, sensor);

				// Fill the denominator of the efficiency histos
				numberOfTracksChannel.get(sensorName).fill(chan);
				numberOfTracksU.get(sensorName).fill(intersect[0]);
				numberOfTracksP.get(sensorName).fill(trackP);

				numberOfTracksChannelCorrected.get(sensorName).fill(chan, weight);
				numberOfTracksUCorrected.get(sensorName).fill(intersect[0], weight);
				numberOfTracksPCorrected.get(sensorName).fill(trackP, weight);

				// Fill electron histograms
				if (q < 0) {
					numberOfTracksChannelEle.get(sensorName).fill(chan);
					numberOfTracksUEle.get(sensorName).fill(intersect[0]);
					numberOfTracksPEle.get(sensorName).fill(trackP);

					numberOfTracksChannelCorrectedEle.get(sensorName).fill(chan, weight);
					numberOfTracksUCorrectedEle.get(sensorName).fill(intersect[0], weight);
					numberOfTracksPCorrectedEle.get(sensorName).fill(trackP, weight);
				} // Fill positron histograms
				else {
					numberOfTracksChannelPos.get(sensorName).fill(chan);
					numberOfTracksUPos.get(sensorName).fill(intersect[0]);
					numberOfTracksPPos.get(sensorName).fill(trackP);

					numberOfTracksChannelCorrectedPos.get(sensorName).fill(chan, weight);
					numberOfTracksUCorrectedPos.get(sensorName).fill(intersect[0], weight);
					numberOfTracksPCorrectedPos.get(sensorName).fill(trackP, weight);
				}

				// Fill the error histos
				errorU.get(sensorName).fill(intError);
				errorUvsV.get(sensorName).fill(intersect[1], intError);
				errorUvsU.get(sensorName).fill(intersect[0], intError);
				errorUvsP.get(sensorName).fill(p.magnitude(), intError);

				if (q < 0) {
					errorUEle.get(sensorName).fill(intError);
					errorUvsVEle.get(sensorName).fill(intersect[1], intError);
					errorUvsUEle.get(sensorName).fill(intersect[0], intError);
					errorUvsPEle.get(sensorName).fill(p.magnitude(), intError);
				} else {
					errorUPos.get(sensorName).fill(intError);
					errorUvsVPos.get(sensorName).fill(intersect[1], intError);
					errorUvsUPos.get(sensorName).fill(intersect[0], intError);
					errorUvsPPos.get(sensorName).fill(p.magnitude(), intError);
				}
				...
				if (lyrHasHit) {
					...
					numberOfTracksWithHitOnMissingLayerU.get(sensorName).fill(intersect[0]);
					numberOfTracksWithHitOnMissingLayerP.get(sensorName).fill(trackP);
					if (q < 0) {
						numberOfTracksWithHitOnMissingLayerChannelEle.get(sensorName).fill(chan);
						numberOfTracksWithHitOnMissingLayerUEle.get(sensorName).fill(intersect[0]);
						numberOfTracksWithHitOnMissingLayerPEle.get(sensorName).fill(trackP);
					} else {
						numberOfTracksWithHitOnMissingLayerChannelPos.get(sensorName).fill(chan);
						numberOfTracksWithHitOnMissingLayerUPos.get(sensorName).fill(intersect[0]);
...
//	}
	...
	private HpsSiSensor getSensor(Track track, int layer) {
		...
		for (HpsSiSensor sensor : sensors) {
			if (sensor.getLayerNumber() == layer) {
				if (sensor.isTopLayer() && tanLambda > 0) {
					return sensor;
				}
				if (sensor.isBottomLayer() && tanLambda < 0) {
					return sensor;
				}
			}
		}
	...
	private Pair<HpsSiSensor, Pair<Integer, Boolean>> getSensorChanInAcceptance(Track track, Double[] localPos,
			int layer) {
		...
		Pair<Integer, Boolean> chanInAcc = this.sensorContainsTrack(locPosVec, sensor);
	...
	public Pair<Integer, Boolean> sensorContainsTrack(Hep3Vector localPos, HpsSiSensor sensor) {
		...
		double width = getSensorLength(sensor);
		Pair<Integer, Hep3Vector> pair = new Pair<>(chan, localPos);
		if (chan < -this.chanExtd || chan > (nChan + this.chanExtd))
			return new Pair<>(chan, false);
		if (Math.abs(localPos.y()) > width / 2) // Checks to see if track is in acceptance of sensor. Computes within
												// sensor
			return new Pair<>(chan, false);
		// frame
		return new Pair<>(chan, true);
	}
