...
/**
 * A class which represents a simple detector as a list of MaterialInfo
...
public class SimpleDetector {
    ...
    // and also to naturally order in z, since MaterialInfo extends Comparable
    TreeSet<MaterialInfo> det = new TreeSet<MaterialInfo>();
    // use a List for fast indexing
    List<MaterialInfo> detList;
    ...
    MaterialInfo response;

    public void addDetectorPlane(MaterialInfo p) {
        if (p.GetZpos() < zMin) {
            zMin = p.GetZpos();
        }
        if (p.GetZpos() > zMax) {
        ...
        det.add(p);
        //update the detList
        detList = new ArrayList<MaterialInfo>(det);
    }

    public List<MaterialInfo> getPlanes() {
        return detList;
    }
