...
public class Efficiency {
    ...
    Efficiency(int nBins, double bin0, double binWidth, String title, String xlabel, String ylabel) {
        N = nBins;
        BW = binWidth;
        B0 = bin0;
        T = title;
        ...
        success = new ArrayList<Integer>(nBins);
    ...
    void entry(float xf, boolean suc) {
        double x = (double) xf;
        int bin = (int) Math.floor((x - B0) / BW);
        if (bin >= 0 && bin < N) {
            int cnt = counts.get(bin);
    ...
    void plot(String fileName, boolean stats, String choice, String txt) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            Y.add(i, (double) success.get(i) / (double) counts.get(i));
            Ex.add(i, BW / 2.);
            Ey.add(i, Math.sqrt((double) success.get(i)) / (double) counts.get(i));
        }
        double mean = -999.;
        double rms = -999.;
        if (nEntry > 0) {
