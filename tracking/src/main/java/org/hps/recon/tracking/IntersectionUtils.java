...
public class IntersectionUtils {
    ...
    public WTrack rkTrack(Hep3Vector ptOnPlane, Hep3Vector normal, Hep3Vector X0, Hep3Vector P0, int Q) {
    ...
    public Hep3Vector rkIntersect(Hep3Vector ptOnPlane, Hep3Vector normal, Hep3Vector X0, Hep3Vector P0, int Q) {
        ...
        WTrack wtrk = rkTrack(ptOnPlane, normal, X0, P0, Q);
        if (debug)
            wtrk._debug = true;
        Hep3Vector temp = wtrk.getHelixAndPlaneIntercept(CoordinateTransformations.transformVectorToTracking(ptOnPlane), normalInTracking, new BasicHep3Vector(0, 0, 1));
        if (temp != null)
            return CoordinateTransformations.transformVectorToDetector(temp);
        return null;
