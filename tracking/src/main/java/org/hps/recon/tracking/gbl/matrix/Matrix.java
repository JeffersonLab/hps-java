...
public class Matrix implements Cloneable, java.io.Serializable {
     ...
     *
     * @return Pointer to the two-dimensional array of matrix elements. TODO should this return a copy?
     */
    ...
    /**
     * Transpose the matrix. The elements of this matrix are changed in place.
     */
    ...
    public void UnitMatrix() {
        // reset A with ones on the diagonal and zeros elsewhere.
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                A[i][j] = (i == j ? 1.0 : 0.0);
            }
    ...
    // }
    //
    public void ResizeTo(int nR, int nC) {
        ...
        // copy back...
        A = new double[nR][nC];
        for (int i = 0; i < Math.min(m, nR); i++) {
            for (int j = 0; j < Math.min(n, nC); j++) {
                A[i][j] = tmp[i][j];
            }
        }
    ...
    public Vector times(Vector b) {
        // new vector has same number of rows as matrix
        Vector v = new Vector(m);
        // loop over rows
        for (int i = 0; i < m; ++i) {
            // loop over columns
            for (int j = 0; j < n; ++j) {
                v.plusEquals(i, A[i][j] * b.get(j));
            }
        }
        return v;
    }
