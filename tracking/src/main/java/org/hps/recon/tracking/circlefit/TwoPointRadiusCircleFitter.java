...
public class TwoPointRadiusCircleFitter {
    ...
    public static CircleFit[] findCircles(double[] p1, double[] p2, double radius) {
        ...
        if (separation == 0.0) {
            ...
            if (_debug) {
                System.out
                        .printf("\nGiven points are opposite ends of a diameter of the circle with center (%.4f,%.4f) and radius %.4f",
                                (p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2, radius);
            }
        } else if (separation > 2 * radius) {
            if (_debug) {
                System.out.printf(
            ...
            results[0] = new CircleFit((p1[0] + p2[0]) / 2 + mirrorDistance * (p1[1] - p2[1]) / separation,
                    (p1[1] + p2[1]) / 2 + mirrorDistance * (p2[0] - p1[0]) / separation, radius);
            results[1] = new CircleFit((p1[0] + p2[0]) / 2 - mirrorDistance * (p1[1] - p2[1]) / separation,
                    (p1[1] + p2[1]) / 2 - mirrorDistance * (p2[0] - p1[0]) / separation, radius);
            if (_debug) {
            ...
            if (_debug) {
                System.out
                        .printf("\nCircle C1 with center (%.4f,%.4f), radius %.4f and Circle C2 with center (%.4f,%.4f), radius %.4f",
                                (p1[0] + p2[0]) / 2 + mirrorDistance * (p1[1] - p2[1]) / separation, (p1[1] + p2[1])
                                        / 2 + mirrorDistance * (p2[0] - p1[0]) / separation, radius, (p1[0] + p2[0])
                                        / 2 - mirrorDistance * (p1[1] - p2[1]) / separation, (p1[1] + p2[1]) / 2
                                        - mirrorDistance * (p2[0] - p1[0]) / separation, radius);
            }
        }
        return results;
    }
