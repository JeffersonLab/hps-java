...
public class GBLOutput extends Driver {
    ...
    private void doMTresiduals(Track trk, Map<HpsSiSensor, TrackerHit> sensorHits) {
        ...
        for (HpsSiSensor sensor : sensorHits.keySet()) {
            ...
            Hep3Vector hitPos = CoordinateTransformations.transformVectorToDetector(new BasicHep3Vector(sensorHits.get(sensor).getPosition()));
            Hep3Vector diff = VecOp.sub(extrapPos, hitPos);
            ITransform3D trans = sensor.getGeometry().getGlobalToLocal();
            trans.transform(diff);

            aida.histogram1D("residual before GBL " + sensor.getName()).fill(diff.y());
        }
    }
    ...
    private void doGBLresiduals(Track trk, Map<HpsSiSensor, TrackerHit> sensorHits) {
        ...
        for (HpsSiSensor sensor : sensorHits.keySet()) {
            ...
            //                }
            aida.histogram2D("predicted v vs u sensor-frame " + sensor.getName()).fill(extrapPosSensor.y(), extrapPosSensor.x());
            ...
            aida.histogram2D("residual after GBL vs v predicted " + sensor.getName()).fill(extrapPosSensor.y(), resSensor.y());
            aida.histogram2D("residual after GBL vs u hit " + sensor.getName()).fill(hitPosSensor.x(), resSensor.y());
            aida.histogram1D("residual after GBL " + sensor.getName()).fill(resSensor.y());
        }
    }
