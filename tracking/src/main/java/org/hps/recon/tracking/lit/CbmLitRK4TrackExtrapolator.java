 ...
 * @version $Id:
 */
public class CbmLitRK4TrackExtrapolator implements CbmLitTrackExtrapolator {

    CbmLitField fField; // magnetic field

    public CbmLitRK4TrackExtrapolator(CbmLitField field) {
        fField = field;
    }

    public LitStatus Extrapolate(CbmLitTrackParam parIn, CbmLitTrackParam parOut, double zOut, double[] F) {
        parOut.copyFrom(parIn);
        LitStatus stat = Extrapolate(parOut, zOut, F);
...
//System.out.println(par);
    ...
    public LitStatus Extrapolate(CbmLitTrackParam parIn, CbmLitTrackParam parOut, DetectorPlane p, double[] F) {
        parOut.copyFrom(parIn);
        LitStatus stat = Extrapolate(parOut, p, F);
        //System.out.println("back"+parOut);
        return stat;
    }

    public LitStatus Extrapolate(CbmLitTrackParam par, DetectorPlane p, double[] F) {
        // extrapolate the track parameters to an arbitrarilty oriented plane...
        // note that we should be reasonably close... so propagate to the z of the plane
        // using the full RK...
        //TODO fix this to propagate either to zmin or zmax, depending...
        ...
        CbmLitTrackParam par1 = new CbmLitTrackParam(par);
//        double[] cIn = par.GetCovMatrix();
//        double[] cOut = new double[15];
//        TransportC(cIn, F1, cOut);
//
...
//        par.SetCovMatrix(cOut);
        ...
        // have now propagated using the full field to the z0 of the plane. 
        ...
        if (!p.inBounds(xOut[0], xOut[1], zOut)) {
            System.err.println("intersection point not in bounds");
            System.out.println(par);
            return LitStatus.kLITERROR;
        }
        return LitStatus.kLITSUCCESS;
