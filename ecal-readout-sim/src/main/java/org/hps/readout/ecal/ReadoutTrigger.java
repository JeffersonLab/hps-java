...
/**
 * Creates distributions from <code>Cluster</code> objects. This is intended to be used on clusters reconstructed from
 * FADC data from the hardware readout and should not be used for Monte Carlo.
 * 
...
public class ReadoutTrigger extends Driver {
    ...
    public void startOfData() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            pairEnergyDifference[i] = aida.histogram1D(plotDir[i] + "Pair Energy Difference" + plotType[i], 88, 0.0,
                    1.1);
            ...
            clusterDistribution[i] = aida.histogram2D(plotDir[i] + "Cluster Seed Distribution" + plotType[i], 46, -23,
                    23, 11, -5.5, 5.5);
            clusterDistribution[i].annotation().addItem("xAxisLabel", "x-Index");
            clusterDistribution[i].annotation().addItem("yAxisLabel", "y-Index");

            pairEnergySum2D[i] = aida.histogram2D(plotDir[i] + "Pair Energy Sum 2D" + plotType[i], 88, 0.0, 2.2, 88,
                    0.0, 2.2);
            pairEnergySum2D[i].annotation().addItem("xAxisLabel", "E1");
            pairEnergySum2D[i].annotation().addItem("yAxisLabel", "E2");

            pairEnergySlope2D[i] = aida.histogram2D(plotDir[i] + "Pair Energy Slope 2D" + plotType[i], 88, 0.0, 1.1,
                    200, 0.0, 400);
    ...
    public void process(EventHeader event) {
        ...
        if (event.hasCollection(Cluster.class, clusterCollectionName)) {
            ...
            pairLoop: for (Cluster[] pair : pairs) {
     ...
     * @return Returns a <code>List</code> collection that contains <code>Cluster</code> arrays of size two.
     */
