...
public class V0Skimmer extends Skimmer {
	...
	if (!event.hasCollection(ReconstructedParticle.class, _V0CandidateCollectionName)) {
		...
		System.out.println(this.getClass().getName()+"::  failed track chi2");
                continue;
            }
            if (electron.getMomentum().magnitude() > _trackPMax || positron.getMomentum().magnitude() > _trackPMax) {
		System.out.println(this.getClass().getName()+"::  failed track momentum");
                continue;
            }
            if (v0.getMomentum().magnitude() > _v0PMax) {
		System.out.println(this.getClass().getName()+"::  failed v0 momentum");
                continue;
            }
		...
		System.out.println(this.getClass().getName()+"::  failed track dt");
                continue;
            }
