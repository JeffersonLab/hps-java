...
public class CosmicPMTFilter extends Driver {
    ...
    public void process(EventHeader event) {
        ...
        if (!event.hasCollection(GenericObject.class, CLASSNAME)) throw new Driver.NextEventException();
    ...
    public float getPulseIntegral(FADCGenericHit hh) {
        float sum = 0;
        for (int samp : hh.getData())
            sum += samp;
        return sum - getPedestal(hh) * hh.getData().length;
    }

    public float getPedestal(FADCGenericHit hh) {
        if (hh.getData().length < NPEDSAMP) throw new java.lang.RuntimeException("Not enough samples for pedestal.");
        float sum = 0;
        for (int isamp = 0; isamp < NPEDSAMP; isamp++)
            sum += hh.getData()[isamp];
        return sum / NPEDSAMP;
    }
}
