...
public class TrackClusterMatcher {
    ...
    private void loadEdges(){
        ...
        for(EcalCrystalPosition pos : positions){
            ...
            //there is a bug in the numbering system.  The next 4 lines compensate for that
            if(Math.abs(iy) ==1 && ix>=1 && ix<=9)
                ix-=10;
            else if(Math.abs(iy) ==1 && ( ix==10 || (ix>=-2 && ix<=-1)))
                ix-=9;
                
            
            
            if(ix<0) ix += 23; else if(ix>0) ix+= 22;
            else continue;
