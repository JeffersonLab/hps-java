...
public class HPSMCParticlePlotsDriver extends Driver {
    ...
    public void startOfData() {
        ...
        fsCountVsEventPlot = aida.histogram1D("MCParticle: Number of Final State Particles vs Event Nr", 501, -0.5,
                500.5);
    ...
    public void process(EventHeader event) {
        ...
        for (int i = 0; i < fsParticles.size(); ++i)
            fsCountVsEventPlot.fill(event.getEventNumber());
        ...
        for (MCParticle fs : fsParticles) {
            ...
            if (ParticleTypeClassifier.isElectron(fsPdg)) {
                ...
                if (fsE > trigThr)
                    fsElectronThetayTrigPlot.fill(Math.abs(thetay));
                ...
                if (fsE > trigThr)
                    fsPositronThetayTrigPlot.fill(Math.abs(thetay));
                ...
                if (fsE > trigThr)
                    fsGammaThetayTrigPlot.fill(Math.abs(thetay));
        ...
        for (MCParticle mcparticle : mcparticles) {
            if (mcparticle.getGeneratorStatus() == MCParticle.FINAL_STATE) {
                double theta = Math.atan2(
                        Math.sqrt(mcparticle.getPX() * mcparticle.getPX() + mcparticle.getPY() * mcparticle.getPY()),
                        mcparticle.getPZ());
                if (theta > 1e-3) {
