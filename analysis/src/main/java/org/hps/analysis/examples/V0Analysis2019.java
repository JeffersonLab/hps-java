...
/**
 * Analysis of events containing a V0 and possibly the recoil electron. Three
 * particles should sum to the beam four-vector.
 *
 * @author Norman A. Graf
 */
public class V0Analysis2019 extends Driver {
...
//        layerCodes.put(4., "L2L2");
    ...
    protected void process(EventHeader event) {
        ...
        if (electrons.size() == 2 && positrons.size() == 1) {
            ReconstructedParticle ele1 = electrons.get(0);
            ReconstructedParticle ele2 = electrons.get(1);
            ReconstructedParticle pos1 = positrons.get(0);
            ...
            // some timing cuts
            // first on the tracks...
            double ele1TrackTime = TrackData.getTrackTime(TrackData.getTrackData(event, ele1.getTracks().get(0)));
            double ele2TrackTime = TrackData.getTrackTime(TrackData.getTrackData(event, ele2.getTracks().get(0)));
            double pos1TrackTime = TrackData.getTrackTime(TrackData.getTrackData(event, pos1.getTracks().get(0)));

            double dtt1 = ele1TrackTime - ele2TrackTime;
            double dtt2 = ele1TrackTime - pos1TrackTime;
            double dtt3 = ele2TrackTime - pos1TrackTime;

            aida.histogram1D("delta track time ele1 ele2", 50, -10., 10.).fill(dtt1);
            aida.histogram1D("delta track time ele1 pos1", 50, -10., 10.).fill(dtt2);
            aida.histogram1D("delta track time ele2 pos1", 50, -10., 10.).fill(dtt3);

            List<BilliorTrack> tracksToVertex = new ArrayList<>();
            tracksToVertex.add(new BilliorTrack(ele1.getTracks().get(0)));
            tracksToVertex.add(new BilliorTrack(pos1.getTracks().get(0)));
            tracksToVertex.add(new BilliorTrack(ele2.getTracks().get(0)));
            BilliorVertex vtx = vtxFitter.fitVertex(tracksToVertex);
            Hep3Vector tridentMom = vtx.getV0Momentum();
            ...
            // require all track times to be within 5 ns
            if (abs(dtt1) < 10 && abs(dtt2) < 10 && abs(dtt3) < 10) {
                aida.histogram1D("trident 4vec energy track time cut", 100, 0.0, 10.).fill(trident.t());
                aida.histogram1D("trident 4vec pX track time cut", 100, -0.2, 0.2).fill(tridentVec.x());
                aida.histogram1D("trident 4vec pY track time cut", 100, -0.2, 0.2).fill(tridentVec.y());
                aida.histogram1D("trident 4vec pZ track time cut", 100, 0., 10.).fill(tridentVec.z());
                aida.histogram1D("Trident x track time cut", 100, -2., 2.).fill(tridentPos.x());
                aida.histogram1D("Trident y track time cut", 100, -2., 2.).fill(tridentPos.y());
                aida.histogram1D("Trident z track time cut", 100, -20., 20.).fill(tridentPos.z());
                aida.histogram1D("Trident pX track time cut", 100, -0.2, 0.2).fill(tridentMom.x());
                aida.histogram1D("Trident pY track time cut", 100, -0.2, 0.2).fill(tridentMom.y());
                aida.histogram1D("Trident pZ track time cut", 100, 0., 10.).fill(tridentMom.z());
                // require three clusters...
                if (ele1.getClusters().size() > 0 && pos1.getClusters().size() > 0 && ele2.getClusters().size() > 0.) {
                    Cluster ele1clus = ele1.getClusters().get(0);
                    Cluster ele2clus = ele2.getClusters().get(0);
                    Cluster pos1clus = pos1.getClusters().get(0);
                    // It seems we can match multiple tracks to the same cluster...
                    if (ele1clus != ele2clus) {
                        aida.histogram1D("trident energy 3 clusters", 100, 0.0, 10.).fill(trident.t());
                        aida.histogram1D("trident 4vec pX 3 clusters", 100, -0.2, 0.2).fill(tridentVec.x());
                        aida.histogram1D("trident 4vec pY 3 clusters", 100, -0.2, 0.2).fill(tridentVec.y());
                        aida.histogram1D("trident 4vec pZ 3 clusters", 100, 0., 10.).fill(tridentVec.z());

//                        System.out.println("ele1clus " + ele1clus);
//                        System.out.println("ele2clus " + ele2clus);
//                        System.out.println("pos1clus " + pos1clus);
                        double t1 = ClusterUtilities.getSeedHitTime(ele1.getClusters().get(0));
                        double t2 = ClusterUtilities.getSeedHitTime(pos1.getClusters().get(0));
                        double t3 = ClusterUtilities.getSeedHitTime(ele2.getClusters().get(0));
                        double dtc1 = t1 - t3;
                        double dtc2 = t1 - t2;
                        double dtc3 = t3 - t2;
                        aida.histogram1D("delta cluster time ele1 ele2", 50, -10., 10.).fill(dtc1);
                        aida.histogram1D("delta cluster time ele1 pos1", 50, -10., 10.).fill(dtc2);
                        aida.histogram1D("delta cluster time ele2 pos1", 50, -10., 10.).fill(dtc3);
                        if (abs(dtc1) < 2 && abs(dtc2) < 2 && abs(dtc3) < 2) {
                            aida.histogram1D("trident 4vec energy 3 clusters dt<2", 100, 0.0, 10.).fill(trident.t());
                            aida.histogram1D("trident 4vec pX 3 clusters dt<2", 100, -0.2, 0.2).fill(tridentVec.x());
                            aida.histogram1D("trident 4vec pY 3 clusters dt<2", 100, -0.2, 0.2).fill(tridentVec.y());
                            aida.histogram1D("trident 4vec pZ 3 clusters dt<2", 100, 0., 10.).fill(tridentVec.z());
                            aida.histogram1D("Trident pX 3 clusters dt<2", 100, -0.2, 0.2).fill(tridentMom.x());
                            aida.histogram1D("Trident pY 3 clusters dt<2", 100, -0.2, 0.2).fill(tridentMom.y());
                            aida.histogram1D("Trident pZ 3 clusters dt<2", 100, 0., 10.).fill(tridentMom.z());
                            aida.histogram1D("delta track time ele1 ele2 3 clusters dt<2", 50, -10., 10.).fill(dtt1);
                            aida.histogram1D("delta track time ele1 pos1 3 clusters dt<2", 50, -10., 10.).fill(dtt2);
                            aida.histogram1D("delta track time ele2 pos1 3 clusters dt<2", 50, -10., 10.).fill(dtt3);
                            double ele1e = ele1.getEnergy();
                            double ele2e = ele2.getEnergy();
                            double pos1e = pos1.getEnergy();
                            aida.histogram2D("ele1 cluster energy vs pos1 energy", 100, 0., 5., 100, 0., 5.).fill(ele1e, pos1e);
                            aida.histogram2D("ele2 cluster energy vs pos1 energy", 100, 0., 5., 100, 0., 5.).fill(ele2e, pos1e);
                            aida.histogram1D("ele1 track momentum", 100, 0., 5.).fill(ele1.getMomentum().magnitude());
                            aida.histogram1D("ele2 track momentum", 100, 0., 5.).fill(ele2.getMomentum().magnitude());
                            aida.histogram1D("pos1 track momentum", 100, 0., 5.).fill(pos1.getMomentum().magnitude());
                            aida.histogram1D("Trident x 3 clusters dt<2", 100, -2., 2.).fill(tridentPos.x());
                            aida.histogram1D("Trident y 3 clusters dt<2", 100, -2., 2.).fill(tridentPos.y());
                            aida.histogram1D("Trident z 3 clusters dt<2", 100, -20., 20.).fill(tridentPos.z());
                        }
                    }
                }
            }
