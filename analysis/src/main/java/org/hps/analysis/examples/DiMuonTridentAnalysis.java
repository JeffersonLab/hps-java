 ...
 */
public class DiMuonTridentAnalysis extends Driver {
...
//    double emass2 = emass * emass;
    ...
    protected void process(EventHeader event) {
        ...
        // sometimes we have MatchedTracks included int the FinalStateParticles, so let's make sure we only count GBL FSPs
        int nGblRps = 0;
        ...
        if (nGblRps == 3) { //rpList.size() == 3) { // note that requring three or less FSPs means we can have at most two V0s in the event
//            String dir = "run " + event.getRunNumber() + " " + V0List.size() + " event V0s";
            String dir = V0List.size() + " event V0s";
...
//                HepLorentzVector beam = VecOp.add(V0.asFourVector(), third.asFourVector());
                HepLorentzVector beam = VecOp.add(makeFourVector(ele, mumass), makeFourVector(pos, mumass));
                if (type.equals("photon")) {
                    beam = VecOp.add(beam, third.asFourVector());
                } else {
                    beam = VecOp.add(beam, makeFourVector(third, emass));
                }
                Hep3Vector beamVec = VecOp.mult(beamAxisRotation, beam.v3());
                double beamEnergy = beam.t();

                aida.histogram1D("V0 + " + type + " energy", 100, 0.5, 10.5).fill(beamEnergy);
                aida.histogram1D("V0 + " + type + " pX", 100, -0.2, 0.2).fill(beamVec.x());
                aida.histogram1D("V0 + " + type + " pY", 100, -0.2, 0.2).fill(beamVec.y());
                aida.histogram1D("V0 + " + type + " pZ", 100, 0.5, 10.5).fill(beamVec.z());
...
//                            }
                            ...
                            p1[1] = vals.get("p1Y");
                            p1[2] = vals.get("p1Z");
                            HepLorentzVector fourVec1 = makeFourVector(p1, mumass);
                            p2[0] = vals.get("p2X");
                            p2[1] = vals.get("p2Y");
                            p2[2] = vals.get("p2Z");
                            HepLorentzVector fourVec2 = makeFourVector(p2, mumass);
...
//                            System.out.println("my trident vector " + tridentVec);
    ...
    private BasicHepLorentzVector makeFourVector(ReconstructedParticle rp, double mass) {
