...
/**
 * Class to strip off "golden events" Currently defined as e+ e- with 
 * six-hit tracks and nothing else in the event.
 */
        ...
        if (event.hasCollection(ReconstructedParticle.class, "UnconstrainedV0Candidates")) {
            ...
            if (vertices.size() > 1 || vertices.isEmpty()) {
                ...
                if (event.get(Track.class, "MatchedTracks").size() > 2) {
                    skipEvent = true;
                    // System.out.println("Thete are: "+event.get(Track.class,
                    // "MatchedTracks").size()+" Matched tracks");
                }
