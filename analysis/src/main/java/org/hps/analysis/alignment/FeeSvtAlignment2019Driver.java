...
public class FeeSvtAlignment2019Driver extends Driver {
    ...
    private double seedCut = 2.9; //= 0.4
...
//        }
    ...
    protected void process(EventHeader event) {
        ...
        List<Cluster> ecalClustersUnCorr = event.get(Cluster.class, "EcalClusters");
        int nClustersUnCorr = ecalClustersUnCorr.size();
        aida.histogram1D("number of Uncorrected Clusters", 10, 0., 10.).fill(ecalClustersUnCorr.size());

        for (Cluster c : ecalClustersUnCorr) {
            aida.histogram1D("ECal Uncorrected cluster energy", 100, 0., 10.).fill(c.getEnergy());
        }
        if (ecalClustersUnCorr.size() != 1) {
            return;
        }

        Cluster cUncorr = ecalClustersUnCorr.get(0);
...
//                    }
                    ...
                    String trackType = TrackType.isGBL(rp.getType()) ? "gbl" : "matched";
                    ...
                    if (c.getPosition()[1] > 0.) {
                        ...
                        aida.histogram1D("Top cluster energy uncorrected", 100, 0.5 * _beamEnergy, 1.5 * _beamEnergy).fill(cUncorr.getEnergy());
                    } else {
                        aida.histogram1D("Bottom cluster energy", 100, 0.5 * _beamEnergy, 1.5 * _beamEnergy).fill(c.getEnergy());
                        aida.histogram1D("Bottom cluster energy uncorrected", 100, 0.5 * _beamEnergy, 1.5 * _beamEnergy).fill(cUncorr.getEnergy());
                    }
                    ...
                    if (debug) {
                        ...
                        aida.cloud1D("uncorr clusterSeedHit energy" + fid).fill(ClusterUtilities.findSeedHit(cUncorr).getCorrectedEnergy());
                        aida.cloud1D("uncorr cluster nHits" + fid).fill(cUncorr.getCalorimeterHits().size());
                        aida.cloud2D("uncorr clusterSeedHit energy vs p" + fid).fill(p, ClusterUtilities.findSeedHit(cUncorr).getCorrectedEnergy());
                        aida.cloud2D("uncorr clusterSeedHit energy vs E" + fid).fill(cUncorr.getEnergy(), ClusterUtilities.findSeedHit(cUncorr).getCorrectedEnergy());
                        aida.cloud2D("uncorr cluster nHits vs p" + fid).fill(p, cUncorr.getCalorimeterHits().size());
                        aida.cloud2D("uncorr cluster time vs p" + fid).fill(p, ClusterUtilities.getSeedHitTime(cUncorr));
                        aida.cloud1D("uncorr energy" + fid).fill(cUncorr.getEnergy());

                    }
                    //WARNING!!!
                    // Redefine cluster here for early reconstruction
                    c = ecalClustersUnCorr.get(0);
