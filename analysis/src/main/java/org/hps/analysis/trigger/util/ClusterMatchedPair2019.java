...
public class ClusterMatchedPair2019 extends Pair<Cluster, VTPCluster> {
     ...
     */
    public ClusterMatchedPair2019(Cluster reconCluster, VTPCluster vtpCluster, byte state) {
        // Set the cluster pairs.
        super(reconCluster, vtpCluster);
        
        // If the state is defined, set it. Otherwise, it is unknown.
        if(state == CLUSTER_STATE_MATCHED || state == CLUSTER_STATE_FAIL_POSITION
                || state == CLUSTER_STATE_FAIL_ENERGY || state == CLUSTER_STATE_FAIL_HIT_COUNT || state == CLUSTER_STATE_FAIL_TIME ) {
            this.state = state;
     ...
     * object.
     */
    public VTPCluster getVTPCluster() {
