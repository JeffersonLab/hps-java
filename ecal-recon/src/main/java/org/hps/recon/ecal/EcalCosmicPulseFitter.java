...
public class EcalCosmicPulseFitter {
     ...
     */
    private boolean fixShapeParameter = true;

    /**
     * don't bother fitting pulses with threshold crossing outside this sample
     * range:
     */
    private int threshRange[] = { 35, 55 };

    /**
     * restrict fit's time0 parameter to this range: (units=samples)
     */
    private int t0limits[] = { 33, 55 };

    /**
     * fit sample range relative to threshold crossing
     */
    private final static int fitRange[] = { -10, 20 };

    /**
     * sample range relative to threshold crossing used to initialize pedestal fit
     * parameter:
     */
    private final static int pedRange[] = { -30, -15 };

    /**
     * if this is positive, it will override individual widths and use one global
     * width (units = samples)
     */
     ...
     * 
     * @param hit
     *            the RawTrackerHit (Mode-1 FADC ECal readout) to be fit
     * @param threshCross
     *            the sample of threshold crossing, used to initialize fit
     *            parameters
     * @param maxADC
     *            the ADC at pulse maximum, used to initialize fit parameters
     * 
     * @return fit result, which can be null if fit fails or certain conditions are
     *         not met.
     */
    public IFitResult fitCosmicPulse(RawTrackerHit hit, int threshCross, double maxADC) {
        ...
        // System.out.println("ii\t"+i+"\tfadc\t"+samples[i]);
        ...
        if (threshCross < threshRange[0] || threshCross > threshRange[1]) {
            ...
            // System.out.println("outside thresh range");
            return null;
        }
        ...
        // System.out.println("pedestal sum"+ped);

        // don't bother trying to fit:
        if (nped == 0)
            return null;
        ped /= nped;
        // System.out.println("pedestal\t"+ped);
        ...
        for (int ii = threshCross + fitRange[0]; ii < threshCross + fitRange[1]; ii++) {
            // System.out.println("ii in fit\t"+ii);
            ...
            // System.out.println("fit point\t"+ii);
        ...
        // don't bother trying to fit:
        // System.out.println("nfitpoints:\t"+nFitPoints);

        if (nFitPoints < 10)
            return null;
        // System.out.println("maxadc:\t"+maxADC+"\tped:\t"+ped);
        ...
        double chi = getChi2(fitResult, fitData);
        // System.out.println("Chi2 \t"+chi);
        // int DOF = fitResult.ndf();
        // System.out.println("Chi2/dof\t"+chi/DOF);
            ...
            if (hitCount % displayHits == 0) {
                ...
                for (int i = 0; i < fitFcn3Pole.parameters().length; i++) {
                    ...
                    System.out.printf("%s\t%.3f\t(+-%.3f)\n", fitResult.fittedFunction().parameterNames()[i],
                            fitResult.fittedFunction().parameters()[i], Math.sqrt(fitResult.covMatrixElement(i, i)));
                }
            }
            double chi2 = getChi2(fitResult, fitData);

            int dof = fitResult.ndf();
            // System.out.println("Chi2/dof\t"+chi2/dof);
