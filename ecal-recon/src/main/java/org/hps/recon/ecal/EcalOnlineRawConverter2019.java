...
/**
 * <code>EcalOnlineRawConverter2019</code> handles the conversion of raw hits of
...
public class EcalOnlineRawConverter2019 {
    ...
    /**
     * Instantiates the <code>EcalOnlineRawConverter</code> and connects it to the
     * <code>ConfigurationManager</code> to receive settings from the DAQ
     * configuration.
     */
    public EcalOnlineRawConverter2019() {
        ...
        ConfigurationManager2019.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                config = ConfigurationManager2019.getInstance().getEcalFADCConfig();
    ...
    public List<CalorimeterHit> HitDtoA(RawTrackerHit hit) {
                ...
                if (config.getMode() == 1) {
                    if (sample < 8) {
                        ...
                        for (int pastSample = 1; pastSample < sample; pastSample++) {
                            if (waveform[pastSample] > absoluteThreshold
                                    && waveform[pastSample - 1] <= absoluteThreshold)
                                continue thresholdLoop;
                        }
                    ...
                    if (sample < config.getNSA() / nsPerSample - 1) {
                        if (waveform[0] > absoluteThreshold)
                            continue thresholdLoop;
                        for (int pastSample = 1; pastSample < sample; pastSample++) {
                            if (waveform[pastSample] > absoluteThreshold
                        ...
                        for (int pastSample = sample
                                - config.getNSA() / nsPerSample; pastSample < sample; pastSample++) {
                            if (waveform[pastSample] > absoluteThreshold
                                    && waveform[pastSample - 1] <= absoluteThreshold)
                                continue thresholdLoop;
                        }
        ...
        for (int thresholdCrossing : thresholdCrossings) {
            ...
            if (energy < 0) {
                Logger.getGlobal().log(Level.INFO, "A hit was produced with " + energy + " GeV energy!");
            }
     ...
     * @return Returns a <code>CalorimeterHit</code> hit object that represents the
     *         raw mode-3 hit with units of energy and a correct time-stamp.
     */
     ...
     * @param timeOffset - The time offset for the hit.
     * @return Returns a <code>CalorimeterHit</code> hit object that represents the
     *         raw mode-7 hit with units of energy and a correct time-stamp.
     */
    ...
    /**
     * Sets whether to use a constant integration window for the the purpose of
     * determining the correct pedestal. This should be used in conjunction with
     * Monte Carlo data during the readout cycle.
     * 
     * @param state - <code>true</code> ignores the size of the readout window when
     *              calculating pedestals, and <code>false</code> accounts for it in
     *              the case of pulse-clipping.
     */
