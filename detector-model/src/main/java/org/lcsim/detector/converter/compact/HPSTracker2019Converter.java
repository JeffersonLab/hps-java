...
import org.lcsim.geometry.compact.converter.HPSTracker2019JavaBuilder;
import org.lcsim.geometry.compact.converter.HPSTrackerBuilder;
import org.lcsim.geometry.compact.converter.HPSTrackerJavaBuilder;
import org.lcsim.geometry.subdetector.HPSTracker2019;

public class HPSTracker2019Converter extends HPSTracker2014v1Converter {

    public HPSTracker2019Converter() {
        super();
    }
    ...
    protected HPSTrackerJavaBuilder initializeBuilder(Element node) {
        return new HPSTracker2019JavaBuilder(_debug, node);
    }
    ...
    protected int getModuleNumber(String surveyVolume) {
        ...
        if (isTopLayer) {
            //if (layer < 3 || layer > 4) {
            if (layer > 4) {
                if (HPSTrackerBuilder.isHoleFromName(surveyVolume)) moduleNumber = 0;
                else moduleNumber = 2;
            } else moduleNumber = 0;
        } else {
            if (layer > 4) {
                if (HPSTrackerBuilder.isHoleFromName(surveyVolume)) moduleNumber = 1;
                else moduleNumber = 3;
            } else moduleNumber = 1;
        }
