    ...
    public void convert(Subdetector subdet, Detector detector) {
        ...
        try {
            for (Iterator i = node.getChildren("layer").iterator(); i.hasNext();) {
                ...
                IDetectorElement layerDe = new DetectorElement(subdet.getName() + "_layer" + layerId,
                        subdet.getDetectorElement(), helper.pack(layerPosId));
                ...
                for (Iterator j = layerElement.getChildren("module_placement").iterator(); j.hasNext();) {
                    ...
                    String modulePlacementName = subdet.getName() + "_" + moduleName + "_layer" + layerId + "_module"
                            + moduleNumber;

                    LogicalVolume lv = modules.get(moduleName);
                    IPhysicalVolume modulePhysVol = new PhysicalVolume(new Transform3D(pos, rot), modulePlacementName,
                            lv, motherVolume, 0);
                    ...
                    SiTrackerModule moduleDe = new SiTrackerModule(modulePlacementName, layerDe, modulePath,
                            moduleNumber);

                    if (debug)
                        System.out.println("created new SiTrackerModule called " + modulePlacementName + " with path: "
                                + modulePath);
                    ...
                    for (IPhysicalVolume componentPhysVol : modulePhysVol.getLogicalVolume().getDaughters()) {
                        ...
                        if (componentPhysVol.getLogicalVolume().getDaughters().size() != 0) {
                            ...
                            expId.setValue(iddict.getFieldIndex("layer"),
                                    helper.getValue(layerDe.getIdentifier(), "layer"));
                            ...
                            if (moduleParameters.get(moduleName).getType()
                                    .equals(HpsTestRunSiSensor.class.getSimpleName())) {
                                sensor = new HpsTestRunSiSensor(sensorNumber, sensorName, moduleDe, sensorPath,
                                        sensorId);
                            } else {
