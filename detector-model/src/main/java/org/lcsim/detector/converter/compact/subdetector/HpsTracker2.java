...
public class HpsTracker2 extends SubdetectorDetectorElement {
        ...
        if (debug) {
            ...
            for (HpsSiSensor sensor : list) {
                System.out.printf("%s: %45s %5d %5d\n", getClass().getSimpleName(), sensor.getName(),
                        sensor.getLayerNumber(), sensor.getModuleNumber());
            }
        }
        return list;
    }
    ...
    private int getMaxLayerNumber() {
        ...
        for (HpsSiSensor sensor : this.getSensors()) {
            if (sensor.getLayerNumber() > maxLayerNumber)
                maxLayerNumber = sensor.getLayerNumber();
        }
    ...
    private int getMaxModuleNumber() {
        int maxModuleID = 0;
        for (HpsSiSensor sensor : this.getSensors()) {
            if (sensor.getModuleNumber() > maxModuleID)
                maxModuleID = sensor.getModuleNumber();
        }
        return maxModuleID;
    }

    /**
     * Method that loops through the collection of {@link HpsSiSensor} and creates stereo layers. A stereo layer is composed of two adjacent sensors (stereo and axial) with the same module number.
     */
