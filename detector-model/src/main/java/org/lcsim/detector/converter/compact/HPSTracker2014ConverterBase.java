...
public abstract class HPSTracker2014ConverterBase extends AbstractSubdetectorConverter {
     ...
     */
    abstract HpsSiSensor createSiSensor(int sensorid, String name, IDetectorElement parent, String support,
            IIdentifier id);
    ...
    public void convert(Subdetector subdet, Detector detector) {
        ...
        builder.setDetectorIdentifierHelper((DetectorIdentifierHelper) subdet.getDetectorElement()
                .getIdentifierHelper());
        ...
        if (_debug)
            System.out.printf("%s: setup physical volumes\n", getClass().getSimpleName());

        setupPhysicalVolumes();

        if (_debug)
            System.out.printf("%s: DONE setup physical volumes\n", getClass().getSimpleName());
        ...
        if (_debug)
            System.out.printf("%s: DONE create stereo layers\n", getClass().getSimpleName());
    ...
    private void setupPhysicalVolumes(JavaSurveyVolume surveyVolume) {
        ...
        if (surveyVolume instanceof JavaGhostSurveyVolume) {
            ...
            if (HPSTrackerBuilder.isBase(surveyVolume.getName())) {
                ...
                if (_debug)
                    System.out.printf("%s: DONE create the base detector element\n", getClass().getSimpleName());

                //}else if (HPSTrackerBuilder.isModule(surveyVolume.getName())) {
                // System.out.printf("%s PF::create the layer detector element\n", getClass().getSimpleName());
                //IDetectorElement fullmoduleDe = createFullModuleDetectorElement(surveyVolume);
                ...
                if (_debug)
                    System.out.printf("%s: DONE create the layer detector element\n", getClass().getSimpleName());
                ...
                if (_debug)
                    System.out.printf("%s: DONE create the module detector element\n", getClass().getSimpleName());

            } else if (HPSTrackerBuilder.isSensor(surveyVolume.getName())) {

                if (_debug)
                    System.out.printf("%s: set sensitive volume for sensor %s\n", getClass().getSimpleName(),
                            surveyVolume.getName());

                createSensorDetectorElement(surveyVolume);

                if (_debug)
                    System.out.printf("%s: DONE set sensitive volume for sensor %s\n", getClass().getSimpleName(),
                            surveyVolume.getName());
                ...
                if (_debug)
                    System.out.printf("%s: DONE create the active sensor detector element\n", getClass()
                            .getSimpleName());

            } else {
                throw new RuntimeException("I don't think I should reach this? Should " + surveyVolume.getName()
                        + " be a ghost?");
            }
        ...
        // add daughters
        if (_debug)
            System.out.printf("%s: add %d daughters to %s\n", this.getClass().getSimpleName(), surveyVolume
                    .getDaughters().size(), surveyVolume.getName());
    ...
    private void createActiveSensorDetectorElement(JavaSurveyVolume surveyVolume) {
        ...
        if (sensorPhysVol == null)
            throw new RuntimeException("cannot find physVol for " + surveyVolume.getName());

        if (_debug)
            System.out.printf("%s: found %s phys vol\n", this.getClass().getSimpleName(), sensorPhysVol.getName());

        // find the full module detector element
        
        IDetectorElement fullmoduleDe = getLayerDetectorElement(surveyVolume);
        ...
        if (moduleDe == null)
            throw new RuntimeException("Cannot find module DE for " + surveyVolume.getName());
        ...
        expId.setValue(iddict.getFieldIndex("layer"),
                builder.getDetectorIdentifierHelper().getValue(layerDe.getIdentifier(), "layer"));
        ...
        if (_debug) {
            System.out
                    .printf("%s: create HpsSiSensor with old layer id %d with sensorNumber %d name %s moduleDe %s sensorPath %s sensor Id %d \n",
                            getClass().getSimpleName(), layerDe.getIdentifier(), sensorNumber, sensorName,
                            moduleDe.getName(), sensorPath, sensorNumber);
        }
        */
        ...
        if (_debug)
            System.out.printf("%s: created sensor %s with layer %d and MP layer %d\n", getClass().getSimpleName(),
                    sensor.getName(), sensor.getLayerNumber(), sensor.getMillepedeId());
    ...
    protected void createTrackerModuleDetectorElement(JavaSurveyVolume surveyVolume, IDetectorElement layerDe) {
        ...
        if (_debug)
            System.out.printf("%s: add module DE to existing ones  \n", getClass().getSimpleName(),
                    modulePlacementName, modulePath, moduleNumber);
    ...
    protected IDetectorElement createAlignableModuleDetectorElement(JavaSurveyVolume surveyVolume) {
        ...
        String volume = "_top";
        int bv = 1;
        if (surveyVolume.getName().endsWith("b")) {
            volume = "_bottom";
            bv = 2;
        }
        ...
        //System.out.printf("%s: PF:: Check Check barrel %d layerPosId layer = %d (compare with new layer %d) %s\n", getClass().getSimpleName(),bv,
        //                layer, HPSTrackerBuilder.getLayerFromVolumeName(surveyVolume.getName()), surveyVolume.getName());
        ...
        if (fullmoduleDe == null) {
            ...
            /*
            fullmoduleDe = new AlignableDetectorElement(builder.getSubdetector().getName() + "_alignable_fullmoduleLayer" + layer+volume, baseDe, builder
                                                        .getDetectorIdentifierHelper().pack(layerPosId),surveyVolume);
            */
        ...
        if (_debug)
            System.out.printf("%s: PF:: created fullmoduleDE  %s  \n", getClass().getSimpleName(), fullmoduleDe.getName());
    ...
    protected IDetectorElement createLayerDetectorElement(JavaSurveyVolume surveyVolume) {
        ...
        //System.out.printf("PFCHECK:: %d\n", builder.getDetectorIdentifierHelper().getBarrelValue());
        ...
        if (layerDe == null) {
            // layerDe = new DetectorElement(builder.getSubdetector().getName() + "_layer" + layer, builder.getSubdetector().getDetectorElement(), builder.getDetectorIdentifierHelper().pack(layerPosId));
            layerDe = new DetectorElement(builder.getSubdetector().getName() + "_layer" + layer, baseDe, builder
                    .getDetectorIdentifierHelper().pack(layerPosId));
    ...
    void createBaseDetectorElement(JavaSurveyVolume surveyVolume) {
        ...
        if (!trackingVolume.getName().contains("tracking")) {
            throw new RuntimeException("base phys mother " + surveyVolume.getPhysMother().getName()
    ...
    public IDetectorElement makeSubdetectorDetectorElement(Detector detector, Subdetector subdetector) {
        ...
        if (_debug)
            System.out.printf("%s: makeSubdetectorDetectorElement for subdetector %s\n", getClass().getSimpleName(),
                    subdetector.getName());

        IDetectorElement subdetectorDE = new HpsTracker2(subdetector.getName(), detector.getDetectorElement());
                ...
                if (info != null) {
                    while ((info = info.parentGeometry()) != null) {
                        ...
                        System.out.printf("%s: Parent LocalToGlobal: \n%s \n", getClass().getSimpleName(),
                                ((Transform3D) info.getLocalToGlobal()).toString());
