...
public abstract class HPSTrackerLCDDBuilder implements IHPSTrackerLCDDBuilder {
    ...
    public void setVisualization() {
        ...
        for (SurveyVolumeImpl g : lcddSurveyVolumes) {
            ...
            if (name.contains("base_plate"))
                g.setVisName("BasePlateVis");
            else if (name.equals("base"))
                g.setVisName("SvtBoxVis");
            else if (name.contains("chamber"))
                g.setVisName("ChamberVis");
            else if (name.contains("support_bottom") || name.contains("support_top"))
                g.setVisName("SupportVolumeVis");
            else if (name.contains("support_plate"))
                g.setVisName("SupportPlateVis");
            else if (name.startsWith("module_")) {
                if (name.endsWith("halfmodule_axial") || name.endsWith("halfmodule_stereo"))
                    g.setVisName("HalfModuleVis");
                else if (name.endsWith("cold"))
                    g.setVisName("ColdBlockVis");
                else if (name.endsWith("lamination"))
                    g.setVisName("KaptonVis");
                else if (name.endsWith("sensor"))
                    g.setVisName("SensorVis");
                else if (name.endsWith("sensor_active"))
                    g.setVisName("SensorVis");
                else if (name.endsWith("cf"))
                    g.setVisName("CarbonFiberVis");
                else if (name.endsWith("hybrid"))
                    g.setVisName("HybridVis");
                else {
