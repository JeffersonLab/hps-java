...
public class EcalWindowPlots extends Driver implements ActionListener {
    ...
    private void setupPlots() {
        ...
        windowPlot = aida.histogram1D(detector.getDetectorName() + " : " + inputCollection + " : Window Mode Data",
        ...
        if (event.hasCollection(RawTrackerHit.class, inputCollection)) {
            ...
            for (RawTrackerHit hit : hits) {
                ...
                if (hit.getADCValues().length != window) {
                    throw new RuntimeException("Hit has unexpected window length " + hit.getADCValues().length
                            + ", not " + window);
                }
