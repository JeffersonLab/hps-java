...
public class BasicMonitoringPlotsDriver extends Driver {
    ...
    public void startOfData() {
        ...
        calHitEnergyH1D = histogramFactory.createHistogram1D(calHitsCollectionName + ": Energy", calHitsCollectionName
                + ": Energy", 200, 0.0, 2.0);
        ...
        rawHitAmplitudeH1D = histogramFactory.createHistogram1D(rawHitsCollectionName + ": Amplitude",
                rawHitsCollectionName + ": Amplitude", 150, 0.0, 15000.0);
        ...
        clusterEnergyH1D = histogramFactory.createHistogram1D(clustersCollectionName + ": Energy",
                clustersCollectionName + ": Energy", 100, 0.0, 3.0);
        ...
        calHitEnergyMapH2D = histogramFactory.createHistogram2D(calHitsCollectionName + ": Energy Map",
                calHitsCollectionName + ": Energy Map", 47, -23.5, 23.5, 11, -5.5, 5.5);
    ...
    public void process(EventHeader event) {
        ...
        if (event.hasCollection(CalorimeterHit.class, calHitsCollectionName)) {
            for (CalorimeterHit hit : event.get(CalorimeterHit.class, calHitsCollectionName)) {
                ...
                calHitEnergyMapH2D.fill(hit.getIdentifierFieldValue("ix"), hit.getIdentifierFieldValue("iy"),
