...
public class SvtTimingInPlots extends Driver {
    ...
    protected void detectorChanged(Detector detector) {
        ...
        for (HpsSiSensor sensor : sensors) {
            ...
            amplitudePlots.put(sensor,
                    histogramFactory.createHistogram1D(sensor.getName() + " - Amplitude", 200, 0, 2000));
            chi2Plots.put(sensor,
                    histogramFactory.createHistogram1D(sensor.getName() + " - Chi^2 Probability", 20, 0, 1));
            t0vAmpPlots.put(sensor, histogramFactory.createHistogram2D(sensor.getName() + " - t0 v Amplitude", 75, -50,
                    100.0, 200, 0, 2000));
            t0vChi2Plots.put(sensor, histogramFactory.createHistogram2D(sensor.getName() + " - t0 v Chi^2 Probability",
                    75, -50, 100.0, 20, 0, 1));
            chi2vAmpPlots.put(sensor, histogramFactory.createHistogram2D(sensor.getName() + " - Chi2 v Amplitude", 20,
                    0, 1, 200, 0, 2000));
            maxSampleNumberPerSensorPlots.put(sensor,
                    histogramFactory.createHistogram1D(sensor.getName() + " - Max Sample Number", 6, 0, 6));
        ...
        maxSampleNumberPerVolumePlots.put("Top",
                histogramFactory.createHistogram1D("SVT Top - Max Sample Number", 6, 0, 6));
        maxSampleNumberPerVolumePlots.put("Bottom",
                histogramFactory.createHistogram1D("SVT Bottom - Max Sample Number", 6, 0, 6));
        plotters.get("Max Sample Per Volume")
                .region(0)
                .plot(maxSampleNumberPerVolumePlots.get("Top"),
                        this.createStyle("Max Sample Number - Top SVT Hits", ""));
        plotters.get("Max Sample Per Volume")
                .region(1)
                .plot(maxSampleNumberPerVolumePlots.get("Bottom"),
                        this.createStyle("Max Sample Number - SVT Bottom Hits", ""));
    ...
    public void process(EventHeader event) {
         ...
         * for (SSPCluster cluster : clusters) {
         * System.out.println("X: " + cluster.getXIndex() + " Y: " + cluster.getYIndex() + " time: " +
         * cluster.getTime());
         * }
         * }
         * }
