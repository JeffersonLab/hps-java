...
public class SVTHitRecoCorrelations extends Driver {
    ...
    protected void detectorChanged(Detector detector) {
         ...
         * 
         * System.out.println("Sensor " + ref_sensor.getName() + " -> ref_s " + ref_s + " layer " + getLayer(ref_sensor)
         * + " phys layer " + ref_l + " axial " + ref_a);
         * //if (!ref_a) continue;
         ...
         * if( a ) {
         * if(1==1) {
         * IHistogram2D corPlot = aida.histogram2D(side[s] + "_" + types[0] + "_layer" + ref_l + "_layer" + l, 50,
         * ymin,ymax, 50, ymin,ymax);
         * plotter.get(s).get(0).region(region).plot(corPlot);
         * }
         * if(doStrips) {
         * //IHistogram2D corPlot1 = aida.histogram2D(side[s] + "_" + types[4] + "_layer" + ref_l + "_layer" + l, 642,
         * 0,641, 642, 0,641);
         * IHistogram2D corPlot1 = aida.histogram2D(side[s] + "_" + types[4] + "_layer" + ref_l + "_layer" + l, 100,
         * 0,641, 100, 0,641);
         * plotter.get(s).get(4).region(region).plot(corPlot1);
         * IHistogram2D corPlot2 = aida.histogram2D(side[s] + "_" + types[6] + "_layer" + ref_l + "_layer" + l, 100,
         * 0,641, 100, 0,641);
         * plotter.get(s).get(6).region(region).plot(corPlot2);
         * }
         * } else {
         * if(1==1) {
         * IHistogram2D corPlot = aida.histogram2D(side[s] + "_" + types[1] + "_layer" + ref_l + "_layer" + l, 50, ymin,
         * ymax, 50, ymin, ymax);
         * plotter.get(s).get(1).region(region).plot(corPlot);
         * }
         * if(doStrips) {
         * //IHistogram2D corPlot1 = aida.histogram2D(side[s] + "_" + types[5] + "_layer" + ref_l + "_layer" + l, 642,
         * 0,641, 642, 0,641);
         * IHistogram2D corPlot1 = aida.histogram2D(side[s] + "_" + types[5] + "_layer" + ref_l + "_layer" + l, 100,
         * 0,641, 100, 0,641);
         * plotter.get(s).get(5).region(region).plot(corPlot1);
         * IHistogram2D corPlot2 = aida.histogram2D(side[s] + "_" + types[7] + "_layer" + ref_l + "_layer" + l, 100,
         * 0,641, 100, 0,641);
         ...
         * ymax=0;
         * }
         * IHistogram2D corPlot = aida.histogram2D(side[s] + "_Y_HTH_layer" + i + "_layer" + ii, 50,ymin, ymax, 50,
         * ymin, ymax);
         * IHistogram2D corPlot1 = aida.histogram2D(side[s] + "_X_HTH_layer" + i + "_layer" + ii, 60, ymin, ymax, 60,
         * ymin,ymax);
    ...
    public void process(EventHeader event) {
        ...
        if (event.hasCollection(RawTrackerHit.class, rawTrackerHitCollectionName)) {
            ...
            System.out.println("The RawTrackerHit collection " + rawTrackerHitCollectionName + " has " + rawHits.size()
                    + " hits.");
            ...
            for (RawTrackerHit ref_hit : rawHits) {
                ...
                int ref_l = ref_helper.getValue(ref_id, "layer"); // 1-10; axial layers are odd layers; stereo layers
                                                                  // are even
                ...
                for (RawTrackerHit hit : rawHits) {
                    ...
                    if (ref_s == s) {
                        ...
                        // if( a) {
                        // if(doStrips) aida.histogram2D(side[s] + "_" + types[6] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_strip,strip);
                        // } else {
                        // if(doStrips) aida.histogram2D(side[s] + "_" + types[7] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_strip,strip);
                        //
                        // }
                    }
        ...
        if (event.hasCollection(SiTrackerHitStrip1D.class, trackerHitCollectionName)) {
            ...
            System.out.println("The SiTrackerHitStrip1D collection " + trackerHitCollectionName + " has "
                    + stripHits.size() + " hits.");
            ...
            for (SiTrackerHitStrip1D ref_cluster : stripHits) {
                ...
                for (SiTrackerHitStrip1D cluster : stripHits) {
                    ...
                    if (ref_s == s) {
                        ...
                        // //System.out.println("side " + side[s]);
                        // if( a) {
                        // if(1==1) aida.histogram2D(side[s] + "_" + types[0] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_cluster.getPosition()[1],cluster.getPosition()[1]);
                        // if(doStrips) aida.histogram2D(side[s] + "_" + types[4] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_strip,strip);
                        // } else {
                        // if(1==1) aida.histogram2D(side[s] + "_" + types[1] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_cluster.getPosition()[1],cluster.getPosition()[1]);
                        // if(doStrips) aida.histogram2D(side[s] + "_" + types[5] + "_layer" + ref_l + "_layer" +
                        // l).fill(ref_strip,strip);
                        //
                        // }
                    }
         ...
         * System.out.println("The HelicalTrackHit collection " + hthOutputCollectionName + " has " + hth.size() +
         * " hits.");
         * 
         ...
         * //System.out.println("Hit name " + name + " layer " + layer + "  x,y , z" + hit.getPosition()[0] + "," +
         * hit.getPosition()[1] + "," + hit.getPosition()[2] );
         * //System.out.println("Hit name " + name + " layer " + layer_ref + "  x,y , z" + hit_ref.getPosition()[0] +
         * "," + hit_ref.getPosition()[1] + "," + hit_ref.getPosition()[2] );
         * 
         * if ( s == ref_s) {
         * aida.histogram2D(side[s] + "_Y_HTH_layer" + layer_ref + "_layer" +
         * layer).fill(hit_ref.getPosition()[1],hit.getPosition()[1]);
         * aida.histogram2D(side[s] + "_X_HTH_layer" + layer_ref + "_layer" +
         * layer).fill(hit_ref.getPosition()[0],hit.getPosition()[0]);
         * }
         * }
         * 
         * }
