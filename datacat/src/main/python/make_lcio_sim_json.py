...
# exclude datasets with these data types
EXCLUDE_DATA_TYPES = ['SIM', 'SIM_READOUT']

def exclude_resource(datafile):
    for exclude in EXCLUDES:
        if exclude in datafile:
            return True
    return False

def exclude_ds(datadict):
    for dt in EXCLUDE_DATA_TYPES:
        if datadict['data_type'] == dt:
            return True
    return False
...
for f in flist:
    datafile = f.strip()
    if exclude_resource(datafile):
        print "Excluding resource '%s'" % datafile
        num_excludes +=1 
        continue            
    try:
        datadict = make_dict(datafile) 
        if exclude_ds(datadict):
            print "Excluding '%s' with data type '%s'" % (datafile, datadict['data_type'])
            num_excludes += 1
            continue
        else:
