...
public class HPSMonitoringKFTracker2021 extends RemoteAidaDriver {
    ...
    // Convert clouds to histograms after this many entries (-1 means do not convert
    // automatically)
    ...
    protected void detectorChanged(Detector detector) {
        ...
        for (HpsSiSensor sensor : sensors) {
            hitsPerSensorPlots.put(SvtPlotUtils.fixSensorNumberLabel(sensor.getName()),
                    aida.histogram1D(SvtPlotUtils.fixSensorNumberLabel(sensor.getName()) + " - Raw Hits", 25, 0, 25));
        ...
        hitCountPlots.put("Raw hit counts/Event", aida.histogram1D("Raw hit counts", 100, 0, 500));
        hitCountPlots.put("SVT top raw hit counts/Event", aida.histogram1D("SVT top raw hit counts", 100, 0, 300));
        ...
        rawTrackerHitsPerEventH1D = aida.histogram1D("Raw Tracker Hits Per Event");
        ...
        clusEnergyH1D = aida.histogram1D("Cluster Energy");
        clusTimeH1D = aida.histogram1D("Cluster Time");
        clusNHitsH1D = aida.histogram1D("Cluster N Hits");
