 ...
 */
public class SvtDigiWithPulserNoSpacingReadoutDriver extends ReadoutDriver {
    //-----------------//
    //--- Constants ---//
    //-----------------//
    ...
    private double pileupCutoff = 0.0;
    ...
    public SvtDigiWithPulserNoSpacingReadoutDriver() {
        add(readoutDriver);
    }
     ...
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#setSamplesAboveThreshold(int)
     * setSamplesAboveThreshold(int)} and the noise threshold may be
     * set with the method {@link
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#setNoiseThreshold(double)
     ...
     * {@link
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#setEnableThresholdCut(boolean)
    ...
    public void detectorChanged(Detector detector) {
	    ...
	    System.out.println(this.getClass().getName()+"::  readout offset = "+readoutOffset+" latency = "+readoutLatency);
        }
    }
	...
	if(debug_){
	    System.out.println("In SvtDigi:: Current time is = "+ReadoutDataManager.getCurrentTime());
	    System.out.println("Number of Sim StripHits for this bunch is "+stripHits.size());
	}
     ...
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver.StripHit StripHit}
     ...
     * {@link
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#samplesAboveThreshold(RawTrackerHit)
     * samplesAboveThreshold(RawTrackerHit)}, {@link
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#pileupCut(RawTrackerHit)
     * pileupCut(RawTrackerHit)}, and {@link
     * org.hps.readout.svt.SvtDigiWithPulserNoSpacingReadoutDriver#badChannelCut(RawTrackerHit)
	...
	//        double firstSample = Math.floor(((triggerTime + 256) - readoutLatency - readoutOffset) / HPSSVTConstants.SAMPLING_INTERVAL)
        ...
        double firstSample = Math.floor(((triggerTime + 0) - readoutLatency - readoutOffset) / HPSSVTConstants.SAMPLING_INTERVAL)
                * HPSSVTConstants.SAMPLING_INTERVAL + readoutOffset;
	...
	if(debug_){
        ...
        for(SiSensor sensor : sensors) {
            ...
            for(int channel = 0; channel < hitQueues.length; channel++) {
                ...
                if(hitQueues[channel] != null) {
		    if(debug_)System.out.println(this.getClass().getName()+":: data in channel = "+channel);
				...
				System.out.println(this.getClass().getName()+":: signal @ time() = "+signalAtTime);
			    }
