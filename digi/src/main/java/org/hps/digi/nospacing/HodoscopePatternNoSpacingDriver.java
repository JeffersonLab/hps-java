 ...
 */
public class HodoscopePatternNoSpacingDriver extends ReadoutDriver {
	...
	//        if (!ReadoutDataManager.checkCollectionStatus(inputCollectionName, localTime + localTimeDisplacement)) {
        //    return;
	// }
	if (!ReadoutDataManager.checkCollectionStatus(inputCollectionName, ReadoutDataManager.getCurrentTime())) {
	    //	    System.out.println(this.getClass().getName()+"::  "+inputCollectionName+" is not ready for this!");
            return;
        }
	...
	//        Collection<CalorimeterHit> fadcHits = ReadoutDataManager.getData(
        ...
        Collection<CalorimeterHit> fadcHits = ReadoutDataManager.getData(
                ReadoutDataManager.getCurrentTime() - (persistentTime - timeEarlierThanEcal), ReadoutDataManager.getCurrentTime() + timeEarlierThanEcal + 4.0,
                inputCollectionName, CalorimeterHit.class);
	
	//	System.out.println(this.getClass().getName()+"::  found "+fadcHits.size()+" fadcHits"); 
	...
	//	System.out.println(this.getClass().getName()+"::  found "+hodoPatterns.size()+" patterns"); 

        // At leaset there is a hodo tilt/cluster hit in any layer, then the pattern list is added into data manager
        if(flag == true){
	    //	    System.out.println(this.getClass().getName()+"::  at least one of the patterns was good!!!");
