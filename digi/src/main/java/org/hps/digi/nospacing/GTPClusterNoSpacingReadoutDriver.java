...
/**
 * Class <code>GTPClusterNoSpacingReadoutDriver</code> produces GTP cluster
 ...
 * org.hps.readout.ecal.updated.GTPClusterNoSpacingReadoutDriver#temporalWindow
 * temporalWindow} and set through the method {@link
 * org.hps.readout.ecal.updated.GTPClusterNoSpacingReadoutDriver#setClusterWindow(int)
 ...
 * {@link
 * org.hps.readout.ecal.updated.GTPClusterNoSpacingReadoutDriver#seedEnergyThreshold
 * seedEnergyThreshold} and set through the method {@link
 * org.hps.readout.ecal.updated.GTPClusterNoSpacingReadoutDriver#setSeedEnergyThreshold(double)
 ...
 * org.hps.readout.ecal.updated.GTPClusterNoSpacingReadoutDriver#setWriteClusterCollection(boolean)
 * setWriteClusterCollection(boolean)} is set to true, the clusters
 * will also be persisted into the output LCIO file.
 */
public class GTPClusterNoSpacingReadoutDriver extends ReadoutDriver {     
    // ==============================================================
    // ==== LCIO Collections ========================================
    // ==============================================================
    ...
    private boolean checkInputStatus=false; //don't check status if running on non-spaced events. 
    ...
    public void process(EventHeader event) {        
        ...
        if(checkInputStatus&&!ReadoutDataManager.checkCollectionStatus(inputCollectionName, ReadoutDataManager.getCurrentTime() +192.0)) {
	    //	    System.out.println("Skipping GTP Readout with because collection doesn't exist at "+(ReadoutDataManager.getCurrentTime() + 192.0));
            return;
        }
        ...
        // TODO: Simplify this?
        Collection<CalorimeterHit> allHits = ReadoutDataManager.getData(ReadoutDataManager.getCurrentTime(), ReadoutDataManager.getCurrentTime() + 192.0, inputCollectionName, CalorimeterHit.class);
	//        Collection<CalorimeterHit> foreHits = ReadoutDataManager.getData(ReadoutDataManager.getCurrentTime() - temporalWindow, ReadoutDataManager.getCurrentTime(), inputCollectionName, CalorimeterHit.class);
        //Collection<CalorimeterHit> postHits = ReadoutDataManager.getData(ReadoutDataManager.getCurrentTime() + 4.0, ReadoutDataManager.getCurrentTime() + temporalWindow + 4.0, inputCollectionName, CalorimeterHit.class);
	...
	//	System.out.println(this.getClass().getName()+"::  "+inputCollectionName+":: local time = "+ReadoutDataManager.getCurrentTime()+" number of seeds = "+allHits.size());
        ...
        seedLoop:
        for(CalorimeterHit seedCandidate : allHits) {
	...
	//	System.out.println(this.getClass().getName()+"::  number of GTP Clusters "+gtpClusters.size());
