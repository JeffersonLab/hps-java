...
//import org.hps.digi.nospacing.NoSpacingTriggerDriver;
...
/**
 * <code>SinglesTrigger2019NoSpacingReadoutDriver</code> simulates an HPS singles trigger
 ...
 */
public class SinglesTrigger2019NoSpacingReadoutDriver extends TriggerDriver {     
    // ==============================================================
    // ==== LCIO Collections ========================================
    // ==============================================================
    /**
    ...
    private double ecalTimeDisplacement = 20.0; //ns
    private double hodoTimeDisplacement = 4.0; //ns
    
    boolean requireHodo=true;
	...
	//	System.out.println(this.getClass().getName()+":: starting process");
        if(ReadoutDataManager.checkCollectionStatus(inputCollectionNameEcal, ReadoutDataManager.getCurrentTime()-ecalTimeDisplacement) && ReadoutDataManager.checkCollectionStatus(inputCollectionNameHodo, ReadoutDataManager.getCurrentTime()-hodoTimeDisplacement)) {
            clusters = ReadoutDataManager.getData(ReadoutDataManager.getCurrentTime()-ecalTimeDisplacement, ReadoutDataManager.getCurrentTime() -ecalTimeDisplacement+ 192.0, inputCollectionNameEcal, Cluster.class);
            hodoPatterns = ReadoutDataManager.getData(ReadoutDataManager.getCurrentTime()-hodoTimeDisplacement, ReadoutDataManager.getCurrentTime() -hodoTimeDisplacement+ 192.0, inputCollectionNameHodo, HodoscopePattern.class);
            
	    //	    System.out.println(this.getClass().getName()+":: number of gtp clusters = "+clusters.size()); 
	    //	    System.out.println(this.getClass().getName()+":: number of hodo patterns = "+hodoPatterns.size()); 
	    if(clusters.size() == 0){
		//		System.out.println(this.getClass().getName()+":: quitting because no gtp clusters");
		return;
	    }

	    if( requireHodo&&hodoPatterns.size() == 0){
		//		System.out.println(this.getClass().getName()+":: quitting because no hodo patterns");
		return;
	    }       
            hodoPatternList = new ArrayList<>(hodoPatterns);
            
        } else {
	    System.out.println(this.getClass().getName()+":: cluster or hodo collection doesn't exist");
	    return;
	}
        ...
        if(isInDeadTime()) {
	    System.out.println(this.getClass().getName()+":: I'm in deadtime ... bailing");
	    return;
	}
		...
		//		System.out.println(this.getClass().getName()+":: this cluster has too few hits ... continue");
                continue;
            }            
		...
		//		System.out.println(this.getClass().getName()+":: this cluster has too low an energy ... continue");

                continue;
            }
            
            if(triggerModule.getCutEn(TriggerModule2019.CLUSTER_TOTAL_ENERGY_HIGH_EN) && !triggerModule.clusterTotalEnergyCutHigh(cluster)) {
		//		System.out.println(this.getClass().getName()+":: this cluster has too HIGH an energy ... continue");
		
                continue;
            }
	    //	    System.out.println(this.getClass().getName()+"::  this cluster survived!");
		    ...
		    //		    System.out.println(this.getClass().getName()+"::  trigger type = "+triggerType+" no trigger because cluster X failed");
                    continue;
                }
		    ...
		    //		    System.out.println(this.getClass().getName()+":: trigger type = "+triggerType+" no trigger because cluster Energy vs X failed");
                    continue;		    
                } 
            }
		...
		//		System.out.println(this.getClass().getName()+"::  trigger type = "+triggerType+"no trigger because hodo matching failed");
                continue;
            }
	    ...
	    //	    System.out.println(this.getClass().getName()+":: found a trigger!!!");
	    ...
	    //            System.out.println(this.getClass().getName()+":: Sending Trigger");
