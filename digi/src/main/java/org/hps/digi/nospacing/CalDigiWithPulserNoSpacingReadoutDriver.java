 ...
 */
public abstract class CalDigiWithPulserNoSpacingReadoutDriver<D extends Subdetector> extends ReadoutDriver {
    ...
    //private static final int CHANNEL_INTEGRATION_DEADTIME = 0;
    /**
     ...
     * readoutOffset}.
     */
    protected int readoutWindow = 48;
    /**
     ...
     */
    //    private int readoutOffset = 0;

    private int readoutOffset = -12;
	...
	if(debug_)System.out.println("DigiReadout:: "+truthHitCollectionName +" local time = "+ReadoutDataManager.getCurrentTime()+" number of hits = "+hits.size());
	    ...
	    if(debug_)
		System.out.println(this.getClass().getName()+":: process:: sim hit energy = "+hit.getRawEnergy()); 
	    ...
	    if(energyAmplitude>debugEnergyThresh && debug_){
		...
		System.out.println(this.getClass().getName()+":: process::  truth energy  = "+energyAmplitude);
		    ...
		    if(energyAmplitude>debugEnergyThresh&&debug_)
			System.out.println(this.getClass().getName()+":: process:: added noise to energy; new energy  = "+energyAmplitude);				
                }
                ...
                // hit occurred.
		if(energyAmplitude>debugEnergyThresh&&debug_)
		    System.out.println(this.getClass().getName()+":: process:: making pulse");
		for(int i = 0; i < pulserDataWindow; i++) {
		    ...
		    // - (ReadoutDataManager.getCurrentTime() + hit.getTime()) - getTimeShiftConditions(hitCellID), hitCellID);
                    ...
                    double voltageDeposition = energyAmplitude * pulseAmplitude((i + 1) * READOUT_PERIOD 
										-  hit.getTime()
										- getTimeShiftConditions(hitCellID)
										, hitCellID);
		    
		    if(energyAmplitude>debugEnergyThresh&&debug_){
			System.out.println(this.getClass().getName()+":: process::  pulse sample i = "+i
					   +"  local time = "+((i + 1) * READOUT_PERIOD -  hit.getTime() - getTimeShiftConditions(hitCellID))
					   +" pulse amplitude = "+pulseAmplitude((i + 1) * READOUT_PERIOD
										 - hit.getTime()
										 - getTimeShiftConditions(hitCellID)
										 , hitCellID));
			System.out.println(this.getClass().getName()+":: process:: writing digitized value for sample = "+i
					   +"  voltage dep value = "+voltageDeposition); 
		    }
                    ...
                    if(energyAmplitude>debugEnergyThresh&&debug_){
			System.out.println(this.getClass().getName()+":: process:: writing digitized value for sample = "+i
					   +"  pre-noise digitized value = "+currentValue);
		    }
		    // If noise should be added, calculate a random value for
			...
			System.out.println(this.getClass().getName()+":: process:: writing digitized value for sample = "+i
                ...
                for(int i = 0; i < pulserDataWindow; i++) {
                    ...
                    // buffer time.
                    double voltageDeposition = energyAmplitude * pulseAmplitude((i + 1) * READOUT_PERIOD
										- hit.getTime() - getTimeShiftConditions(hitCellID), hitCellID);
    ...
    private void readHits(List<RawCalorimeterHit> newHits, List<LCRelation> newTruthRelations) {
	    ...
	    if(pedestalSubtractedValue > integrationThreshold && debug_){
		...
		if(debug_)System.out.println(this.getClass().getName()+":: readHits::  Found a hit above threshold = "+cellID);
		...
		if(debug_)System.out.println(this.getClass().getName()+":: readHits::  sum before this sample = "+sumBefore);
            ...
            if(sum != null) {
		 if(debug_)System.out.println(this.getClass().getName()+":: readHits::  integration is ongoing..."); 
		    ...
		    if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 1:  DEADTIME>NSA"); 
		    ...
		    if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case2:  DEADTIME==NSA"); 
                    // Continue integration until NSA, the threshold-crossing sample has been added before.
                    if (channelIntegrationTimeMap.get(cellID) + numSamplesAfter - 1 >= readoutCounter) {
                        channelIntegrationADCMap.get(cellID).add(adcBuffer.getValue(0));
		    ...
		    if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  DEADTIME<NSA  "+cellID); 
                    if (channelIntegrationTimeMap.get(cellID) + CHANNEL_INTEGRATION_DEADTIME - 1 >= readoutCounter) {
			 if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  time + deadtime - 1 >= readoutCounter"+(channelIntegrationTimeMap.get(cellID) + CHANNEL_INTEGRATION_DEADTIME - 1)+">="+readoutCounter+"....just keep integrating  "+cellID);
			     ...
			     if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  time + deadtime - 1 >= readoutCounter ... at deadtime limit and below threshold, setting new integration flag to true");
                            flagStartNewIntegration.put(cellID, true);
			}
                    }  
                    else if (channelIntegrationTimeMap.get(cellID) + numSamplesAfter - 1 >= readoutCounter) {
			 if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  time + NSA - 1 >= readoutCounter"+(channelIntegrationTimeMap.get(cellID) + numSamplesAfter - 1)+">="+readoutCounter+"....decide what to do  "+cellID);
                        if(flagStartNewIntegration.get(cellID) == true) { // Flag for previous sample is true
			     if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  new integration flag is true; starting new integration  "+cellID); 
                            if(pedestalSubtractedValue <= integrationThreshold) { // If sample is less than threshold, then do not start new integration
                                ...
                                // Add a new calorimeter hit.
				if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3: making new hit after new integration flag is true because sample is over threshold and new integration is starting!!! "+cellID); 
                                RawCalorimeterHit newHit = new BaseRawCalorimeterHit(cellID, sum,
                                        64 * channelIntegrationTimeMap.get(cellID));
                                newHits.add(newHit);
                                integrationTime = channelIntegrationTimeMap.get(cellID) * 4 + 2;
			     ...
			     if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3: new integration flag is false; just add new sample "+cellID); 
                            channelIntegrationADCMap.get(cellID).add(adcBuffer.getValue(0));

                            // Add the new ADC sample.
                            channelIntegrationSumMap.put(cellID, sum + adcBuffer.getValue(0));
			     if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3: new integration flag is false:  integration sum  = "+(sum + adcBuffer.getValue(0))); 
                            // Add the new truth information, if trigger
                            // path truth output is enabled.
                            if (writeTriggerTruth) {
                                channelIntegrationTruthMap.get(cellID).addAll(truthBufferMap.get(cellID).getValue(0));
				 ...
				 if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3: new integration flag is false:  went below threshold, setting flag to true");
                                flagStartNewIntegration.put(cellID, true);
			    }
                        }  
                    }
			 ...
			 if(debug_)System.out.println(this.getClass().getName()+":: readHits::  case 3:  reached NSA + 1; adding new hit "+cellID); 
                        // Add a new calorimeter hit.
                        RawCalorimeterHit newHit = new BaseRawCalorimeterHit(cellID, sum,
                                64 * channelIntegrationTimeMap.get(cellID));
                        newHits.add(newHit);
	 ...
	 if(debug_)System.out.println("DigiReadout:: "+ outputHitCollectionName+" local time = "+(ReadoutDataManager.getCurrentTime()+readoutTime())+" adding trigger hits = "+newHits.size());
	ReadoutDataManager.addData(outputHitCollectionName, ReadoutDataManager.getCurrentTime()+readoutTime(), newHits, RawCalorimeterHit.class);	
	newHits.clear();  //remove newHits since we've already put it in data manager
        if(writeTriggerTruth) {
            ReadoutDataManager.addData(triggerTruthRelationsCollectionName, integrationTime, newTruthRelations, LCRelation.class);
	    newTruthRelations.clear();
        }
	...
	if(debug_)System.out.println(this.getClass().getName()+":: getting mode = "+mode+" hits on trigger time = "+triggerTime);
        if(mode == 7) {
	...
	if(debug_)System.out.println(this.getClass().getName()+":: getting mode1Hits for trigger time = "+triggerTime+" and readout window = "+readoutWindow);
        // Iterate over each channel.
        for(Long cellID : adcBufferMap.keySet()) {
            // Get the ADC values at the time of the trigger.	    
            short[] adcValues = getTriggerADCValues(cellID, triggerTime);
	    if(debug_){
		Collection<SimCalorimeterHit> simHits=getTriggerTruthValues(cellID, triggerTime);
		if(simHits.size()==0)
		    System.out.println(this.getClass().getName()+":: no sim cal hits in this channel");
		else{
		    for( SimCalorimeterHit hit: simHits)
			System.out.println(this.getClass().getName()+":: sim cal hit in this channel with energy = "+hit.getRawEnergy());
		}
	    }
		    ...
		    if(debug_)System.out.println(this.getClass().getName()+":: found an adc value above threshold for cellID = "+cellID);
	...
	if(debug_)System.out.println(this.getClass().getName()+"::  getTriggerADCValues::  latency = "+readoutLatency);
        for(int i = 0; i < readoutWindow; i++) {	    
            adcValues[i] = (short) pipeline.getValue(-(readoutLatency - i - 1)).intValue();
	    if(debug_)
		System.out.println(this.getClass().getName()+"::  getTriggerADCValues:: "+" pipeline index = "+ (-(readoutLatency - i - 1))
				   +"  adcValue["+i+"] = "+adcValues[i]);
        }
